...
//! - `Yuva444p10` (10-bit, both u8 RGBA + u16 RGBA outputs)
//!
//! **Scope / BITS choice**: only BITS=10 is benchmarked for the high-bit family.
//! BITS=9/10/12/14 all exercise the BITS-generic Q15 i32 chroma kernel and are
//! expected to show very similar A+ speedup ratios. BITS=16 exercises the dedicated
//! i64 chroma-widened family (separate from the Q15 i32 template) and is where the
//! A+ win should be largest — but it is not yet included here because the i64
//! `Yuva444p16` SIMD path (Ship 8b‑5b) has not landed; the i64 path is scalar-only
//! today. Add a `yuva444p16_a_plus_combo_u16` group once Ship 8b‑5b lands and the
//! full SIMD i64 speedup is available to measure.
//!
...
use colconv::{
  ...
  row::{
    yuv_444_to_rgb_row, yuv444p10_to_rgb_row, yuv444p10_to_rgb_u16_row, yuva444p_to_rgba_row,
    yuva444p10_to_rgba_row, yuva444p10_to_rgba_u16_row,
  },
  sinker::MixedSinker,
  yuv::{Yuva444p, Yuva444p10, yuva444p_to, yuva444p10_to},
};
...
criterion_main!(benches_8bit, benches_10bit_u8, benches_10bit_u16);
