//! Per‑row packed YUV 4:2:2 (YVYU) → packed RGB throughput baseline.
//!
//! YVYU is the FFmpeg `AV_PIX_FMT_YVYU422` layout — `Y0 V0 Y1 U0` repeating,
//! two pixels per 4 bytes (V/U swap relative to YUYV). Two variants per
...
use colconv::{ColorMatrix, row::yvyu422_to_rgb_row};
...
fn bench(c: &mut Criterion) {
  ...
  let mut group = c.benchmark_group("yvyu422_to_rgb_row");

  for &w in WIDTHS {
    let mut packed = std::vec![0u8; w * 2];
    fill_pseudo_random(&mut packed, 0x3333);
