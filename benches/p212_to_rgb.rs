//! Per‑row P212 (semi‑planar 4:2:2, 12‑bit, high‑bit‑packed) → RGB
//! throughput baseline. Reuses 4:2:0 P012 row primitives.
...
fn fill_pseudo_random_p212(buf: &mut [u16], seed: u32) {
  let mut state = seed;
  for b in buf {
...
fn bench(c: &mut Criterion) {
  ...
  let mut group_u8 = c.benchmark_group("p212_to_rgb_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; w];
    fill_pseudo_random_p212(&mut y, 0x1111);
    fill_pseudo_random_p212(&mut uv, 0x2222);
  ...
  let mut group_u16 = c.benchmark_group("p212_to_rgb_u16_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; w];
    fill_pseudo_random_p212(&mut y, 0x1111);
