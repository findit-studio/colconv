//! Per‑row P216 (semi‑planar 4:2:2, 16‑bit) → RGB throughput baseline.
//! Reuses 4:2:0 P016 row primitives (parallel i64-chroma family for
//! u16 output).
...
fn fill_pseudo_random_p216(buf: &mut [u16], seed: u32) {
  let mut state = seed;
  for b in buf {
...
fn bench(c: &mut Criterion) {
  ...
  let mut group_u8 = c.benchmark_group("p216_to_rgb_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; w];
    fill_pseudo_random_p216(&mut y, 0x1111);
    fill_pseudo_random_p216(&mut uv, 0x2222);
  ...
  let mut group_u16 = c.benchmark_group("p216_to_rgb_u16_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; w];
    fill_pseudo_random_p216(&mut y, 0x1111);
