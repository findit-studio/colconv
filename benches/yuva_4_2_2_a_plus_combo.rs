...
//! BITS=9 is niche, BITS=12 and BITS=16 also exist for Yuva422p (unlike Yuva420p
//! where FFmpeg has no 12/14-bit variants). BITS=10 is the most common path
//! (ProRes 422 LT/HQ, DNxHD/HR, HDR10 4:2:2) and fully exercises the
//! BITS-generic Q15 i32 chroma kernel. BITS=12 would also exercise the Q15 i32
//! path (same template, different `BITS` constant) and is not expected to show a
//! materially different A+ speedup ratio.
//!
...
use colconv::{
  ...
  sinker::MixedSinker,
  yuv::{Yuva422p, Yuva422p10, yuva422p_to, yuva422p10_to},
};
