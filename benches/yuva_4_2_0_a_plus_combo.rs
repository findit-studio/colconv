...
//! BITS=9 is niche (AVC High 9 only), BITS=16 runs the i64 kernel family instead
//! of the BITS-generic Q15 i32 template that BITS=10 exercises, and BITS=12/14
//! do not exist for YUVA 4:2:0 (FFmpeg only ships yuva420p9le / yuva420p10le /
//! yuva420p16le). BITS=10 is the most common HDR10 path and fully exercises the
//! BITS-generic i32 chroma kernel.
//!
...
//! BITS=16 (where `coeff × u_d` can overflow i32). The A+ speedup here removes
//! one full chroma kernel invocation (i32 path), not an i64 one.
//!
...
use colconv::{
  ...
  row::{
    yuv_420_to_rgb_row, yuv420p10_to_rgb_row, yuv420p10_to_rgb_u16_row, yuva420p_to_rgba_row,
    yuva420p10_to_rgba_row, yuva420p10_to_rgba_u16_row,
  },
  sinker::MixedSinker,
  yuv::{Yuva420p, Yuva420p10, yuva420p_to, yuva420p10_to},
};
