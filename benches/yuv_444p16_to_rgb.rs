//! Per-row YUV 4:4:4 16-bit planar → packed RGB throughput baseline.
//!
...
use colconv::{
  ColorMatrix,
  row::{yuv444p16_to_rgb_row, yuv444p16_to_rgb_u16_row},
};
...
fn bench(c: &mut Criterion) {
  ...
  let mut group_u8 = c.benchmark_group("yuv444p16_to_rgb_row");

  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut u = std::vec![0u16; w];
    let mut v = std::vec![0u16; w];
  ...
  let mut group_u16 = c.benchmark_group("yuv444p16_to_rgb_u16_row");
