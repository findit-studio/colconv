//! Per‑row P412 (semi‑planar 4:4:4, 12‑bit, high‑bit‑packed) → RGB
//! throughput baseline. Uses the `p_n_444_to_rgb_*<12>` SIMD family.
...
use colconv::{
  ColorMatrix,
  row::{p412_to_rgb_row, p412_to_rgb_u16_row},
};

fn fill_pseudo_random_p412(buf: &mut [u16], seed: u32) {
  let mut state = seed;
  for b in buf {
...
fn bench(c: &mut Criterion) {
  ...
  let mut group_u8 = c.benchmark_group("p412_to_rgb_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; 2 * w];
    fill_pseudo_random_p412(&mut y, 0x1111);
    fill_pseudo_random_p412(&mut uv, 0x2222);
  ...
  let mut group_u16 = c.benchmark_group("p412_to_rgb_u16_row");
  for &w in WIDTHS {
    let mut y = std::vec![0u16; w];
    let mut uv = std::vec![0u16; 2 * w];
    fill_pseudo_random_p412(&mut y, 0x1111);
