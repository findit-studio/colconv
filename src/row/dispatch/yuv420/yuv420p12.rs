...
/// Converts one row of **12-bit** YUV 4:2:0 **directly** to planar
/// HSV bytes (OpenCV `cv2.COLOR_RGB2HSV` encoding: `H ∈ [0, 179]`,
/// `S, V ∈ [0, 255]`), without materializing a source-width RGB row.
/// Byte-identical to `rgb_to_hsv_row(yuv420p12_to_rgb_row_endian
...
#[cfg_attr(not(tarpaulin), inline(always))]
#[allow(clippy::too_many_arguments)]
pub fn yuv420p12_to_hsv_row_endian(
  y: &[u16],
  u_half: &[u16],
  v_half: &[u16],
  h_out: &mut [u8],
  ...
  if big_endian {
    super::yuv_420p_n_to_hsv_row::<12, true>(
      y, u_half, v_half, h_out, s_out, v_out, width, matrix, full_range, use_simd,
    );
  } else {
    super::yuv_420p_n_to_hsv_row::<12, false>(
      y, u_half, v_half, h_out, s_out, v_out, width, matrix, full_range, use_simd,
    );
  }
}
