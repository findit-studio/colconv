...
/// Converts one row of AYUV64 **directly** to planar HSV bytes (OpenCV
/// `cv2.COLOR_RGB2HSV` encoding: `H ∈ [0, 179]`, `S, V ∈ [0, 255]`),
/// without materializing a source-width RGB row. Byte-identical to
/// `rgb_to_hsv_row(ayuv64_to_rgb_row(...))` within the selected tier —
/// the SIMD path stages a fixed 64-pixel 8-bit RGB chunk internally.
/// Source α is dropped (HSV is colour-only). See
/// `scalar::ayuv64_to_hsv_row` for the reference. `use_simd = false`
/// forces the scalar reference path; `be_input = true` selects the
/// big-endian wire variant.
#[cfg_attr(not(tarpaulin), inline(always))]
#[allow(clippy::too_many_arguments)]
pub fn ayuv64_to_hsv_row(
  packed: &[u16],
  h_out: &mut [u8],
  s_out: &mut [u8],
  v_out: &mut [u8],
    ...
    cfg_select! {
      target_arch = "aarch64" => {
        if neon_available() {
          ...
          if be_input {
            unsafe { arch::neon::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          } else {
            unsafe { arch::neon::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          }
          return;
        }
      },
      target_arch = "x86_64" => {
        if avx512_available() {
          // SAFETY: AVX-512BW verified.
          if be_input {
            unsafe { arch::x86_avx512::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          } else {
            unsafe { arch::x86_avx512::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          }
          return;
        }
        if avx2_available() {
          // SAFETY: AVX2 verified.
          if be_input {
            unsafe { arch::x86_avx2::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          } else {
            unsafe { arch::x86_avx2::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          }
          return;
        }
        if sse41_available() {
          // SAFETY: SSE4.1 verified.
          if be_input {
            unsafe { arch::x86_sse41::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          } else {
            unsafe { arch::x86_sse41::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          }
          return;
        }
      },
      target_arch = "wasm32" => {
        if simd128_available() {
          // SAFETY: simd128 compile-time verified.
          if be_input {
            unsafe { arch::wasm_simd128::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          } else {
            unsafe { arch::wasm_simd128::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          }
          return;
        }
      },
  ...
  if be_input {
    scalar::ayuv64_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range);
  } else {
    scalar::ayuv64_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range);
  }
}
...
mod tests {
  ...
  /// Pack one AYUV64 pixel (host-native u16 quadruple) from explicit A / Y / U
  /// / V samples (16-bit native, no shift required).
  fn pack_ayuv64(a: u16, y: u16, u: u16, v: u16) -> [u16; 4] {
    [a, y, u, v]
  }

  /// Build a `Vec<u16>` AYUV64 row of `width` pixels with neutral chroma
  /// (U=V=32768) and the given Y / alpha values, in LE-encoded byte form so
  /// dispatchers with `be_input = false` recover the intended logical values
  ...
  /// Build a `Vec<u16>` AYUV64 row in BE-encoded byte form so dispatchers
  /// with `be_input = true` recover the intended logical values on both LE
  /// and BE hosts.
