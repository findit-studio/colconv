...
/// LE-only wrapper around [`y212_to_rgb_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_rgb_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgb_row(
  ...
  y212_to_rgb_row_endian(packed, rgb_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y212 to packed RGBA (u8) with `α = 0xFF`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgba_row_endian(
  packed: &[u16],
  rgba_out: &mut [u8],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y212_to_rgba_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_rgba_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgba_row(
  ...
  y212_to_rgba_row_endian(packed, rgba_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y212 to packed `u16` RGB at native 12-bit
/// depth (low-bit-packed, `[0, 4095]`).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgb_u16_row_endian(
  packed: &[u16],
  rgb_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y212_to_rgb_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_rgb_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgb_u16_row(
  ...
  y212_to_rgb_u16_row_endian(packed, rgb_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y212 to packed `u16` RGBA at native 12-bit
/// depth with `α = 4095` (12-bit opaque maximum).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgba_u16_row_endian(
  packed: &[u16],
  rgba_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y212_to_rgba_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_rgba_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_rgba_u16_row(
  ...
  y212_to_rgba_u16_row_endian(packed, rgba_out, width, matrix, full_range, use_simd, false)
}

/// Extracts one row of 8-bit luma from a packed Y212 buffer.
/// Y values are downshifted from 12-bit to 8-bit via `>> 4`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_luma_row_endian(
  packed: &[u16],
  luma_out: &mut [u8],
  width: usize,
  use_simd: bool,
...
/// LE-only wrapper around [`y212_to_luma_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_luma_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_luma_row(packed: &[u16], luma_out: &mut [u8], width: usize, use_simd: bool) {
  y212_to_luma_row_endian(packed, luma_out, width, use_simd, false)
}
...
/// its low 12 bits).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_luma_u16_row_endian(
  packed: &[u16],
  luma_out: &mut [u16],
  width: usize,
  use_simd: bool,
...
/// LE-only wrapper around [`y212_to_luma_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y212_to_luma_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y212_to_luma_u16_row(packed: &[u16], luma_out: &mut [u16], width: usize, use_simd: bool) {
  y212_to_luma_u16_row_endian(packed, luma_out, width, use_simd, false)
}

/// Converts one row of **Y212** (packed YUV 4:2:2, 12-bit, MSB-aligned)
...
pub fn y212_to_hsv_row_endian(
    ...
    cfg_select! {
      target_arch = "aarch64" => {
        if neon_available() {
          // SAFETY: NEON verified.
          dispatch_be!(
            unsafe { arch::neon::y2xx_n_to_hsv_row::<12, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::neon::y2xx_n_to_hsv_row::<12, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
      target_arch = "x86_64" => {
        if avx512_available() {
          // SAFETY: AVX-512BW verified.
          dispatch_be!(
            unsafe { arch::x86_avx512::y2xx_n_to_hsv_row::<12, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_avx512::y2xx_n_to_hsv_row::<12, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
        if avx2_available() {
          // SAFETY: AVX2 verified.
          dispatch_be!(
            unsafe { arch::x86_avx2::y2xx_n_to_hsv_row::<12, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_avx2::y2xx_n_to_hsv_row::<12, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
        if sse41_available() {
          // SAFETY: SSE4.1 verified.
          dispatch_be!(
            unsafe { arch::x86_sse41::y2xx_n_to_hsv_row::<12, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_sse41::y2xx_n_to_hsv_row::<12, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
      target_arch = "wasm32" => {
        if simd128_available() {
          // SAFETY: simd128 compile-time verified.
          dispatch_be!(
            unsafe { arch::wasm_simd128::y2xx_n_to_hsv_row::<12, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::wasm_simd128::y2xx_n_to_hsv_row::<12, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
  ...
  dispatch_be!(
    scalar::y212_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range),
    scalar::y212_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range)
  );
}
