//! Y210 (Tier 4 packed YUV 4:2:2 10-bit, MSB-aligned u16 quadruples)
//! dispatchers (Ship 11b).
//!
//! Six entries: `y210_to_{rgb,rgba}_row` (u8) and the matching
//! `_u16` variants for native-depth output, plus
//! `y210_to_luma_row` / `y210_to_luma_u16_row` for direct luma
//! extraction. Routes through the standard `cfg_select!` per-arch
//! block; `use_simd = false` forces scalar.
//!
//! Y210 shares its kernel family with Y212 (Ship 11c) — both are
//! BITS-template specializations of the `y2xx_n_*` kernels. The
//! public dispatchers hard-wire `BITS = 10` and split the
...
/// LE-only wrapper around [`y210_to_rgb_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_rgb_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgb_row(
  ...
  y210_to_rgb_row_endian(packed, rgb_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y210 to packed RGBA (u8) with `α = 0xFF`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgba_row_endian(
  packed: &[u16],
  rgba_out: &mut [u8],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y210_to_rgba_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_rgba_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgba_row(
  packed: &[u16],
  rgba_out: &mut [u8],
  width: usize,
  matrix: ColorMatrix,
  ...
  y210_to_rgba_row_endian(packed, rgba_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y210 to packed `u16` RGB at native 10-bit
/// depth (low-bit-packed, `[0, 1023]`).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgb_u16_row_endian(
  packed: &[u16],
  rgb_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y210_to_rgb_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_rgb_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgb_u16_row(
  packed: &[u16],
  rgb_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
  ...
  y210_to_rgb_u16_row_endian(packed, rgb_out, width, matrix, full_range, use_simd, false)
}

/// Converts one row of Y210 to packed `u16` RGBA at native 10-bit
/// depth with `α = 1023` (10-bit opaque maximum).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgba_u16_row_endian(
  packed: &[u16],
  rgba_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
...
/// LE-only wrapper around [`y210_to_rgba_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_rgba_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_rgba_u16_row(
  packed: &[u16],
  rgba_out: &mut [u16],
  width: usize,
  matrix: ColorMatrix,
  ...
  y210_to_rgba_u16_row_endian(packed, rgba_out, width, matrix, full_range, use_simd, false)
}

/// Extracts one row of 8-bit luma from a packed Y210 buffer.
/// Y values are downshifted from 10-bit to 8-bit via `>> 2`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_luma_row_endian(
  packed: &[u16],
  luma_out: &mut [u8],
  width: usize,
  use_simd: bool,
...
/// LE-only wrapper around [`y210_to_luma_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_luma_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_luma_row(packed: &[u16], luma_out: &mut [u8], width: usize, use_simd: bool) {
  y210_to_luma_row_endian(packed, luma_out, width, use_simd, false)
}
...
/// its low 10 bits).
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_luma_u16_row_endian(
  packed: &[u16],
  luma_out: &mut [u16],
  width: usize,
  use_simd: bool,
...
/// LE-only wrapper around [`y210_to_luma_u16_row_endian`]; preserves the
/// pre-endian-aware public signature so existing little-endian
/// callers compile unchanged. Equivalent to `y210_to_luma_u16_row_endian(
/// ..., big_endian = false)`.
#[cfg_attr(not(tarpaulin), inline(always))]
pub fn y210_to_luma_u16_row(packed: &[u16], luma_out: &mut [u16], width: usize, use_simd: bool) {
  y210_to_luma_u16_row_endian(packed, luma_out, width, use_simd, false)
}

/// Converts one row of **Y210** (packed YUV 4:2:2, 10-bit, MSB-aligned)
/// **directly** to planar HSV bytes (OpenCV `cv2.COLOR_RGB2HSV` encoding:
/// `H ∈ [0, 179]`, `S, V ∈ [0, 255]`), without materializing a
/// source-width RGB row. Byte-identical to
/// `rgb_to_hsv_row(y210_to_rgb_row_endian(...))` within the selected tier
/// — the SIMD path stages a fixed 64-pixel 8-bit RGB chunk internally.
/// See `scalar::y210_to_hsv_row::<false>` for the reference.
/// `use_simd = false` forces the scalar reference path. `big_endian = true`
/// selects big-endian wire decoding.
#[cfg_attr(not(tarpaulin), inline(always))]
#[allow(clippy::too_many_arguments)]
pub fn y210_to_hsv_row_endian(
  packed: &[u16],
  h_out: &mut [u8],
  s_out: &mut [u8],
  v_out: &mut [u8],
    ...
    cfg_select! {
      target_arch = "aarch64" => {
        if neon_available() {
          // SAFETY: NEON verified.
          dispatch_be!(
            unsafe { arch::neon::y2xx_n_to_hsv_row::<10, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::neon::y2xx_n_to_hsv_row::<10, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
      target_arch = "x86_64" => {
        if avx512_available() {
          // SAFETY: AVX-512BW verified.
          dispatch_be!(
            unsafe { arch::x86_avx512::y2xx_n_to_hsv_row::<10, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_avx512::y2xx_n_to_hsv_row::<10, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
        if avx2_available() {
          // SAFETY: AVX2 verified.
          dispatch_be!(
            unsafe { arch::x86_avx2::y2xx_n_to_hsv_row::<10, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_avx2::y2xx_n_to_hsv_row::<10, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
        if sse41_available() {
          // SAFETY: SSE4.1 verified.
          dispatch_be!(
            unsafe { arch::x86_sse41::y2xx_n_to_hsv_row::<10, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::x86_sse41::y2xx_n_to_hsv_row::<10, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
      target_arch = "wasm32" => {
        if simd128_available() {
          // SAFETY: simd128 compile-time verified.
          dispatch_be!(
            unsafe { arch::wasm_simd128::y2xx_n_to_hsv_row::<10, false>(packed, h_out, s_out, v_out, width, matrix, full_range); },
            unsafe { arch::wasm_simd128::y2xx_n_to_hsv_row::<10, true>(packed, h_out, s_out, v_out, width, matrix, full_range); }
          );
          return;
        }
      },
  ...
  dispatch_be!(
    scalar::y210_to_hsv_row::<false>(packed, h_out, s_out, v_out, width, matrix, full_range),
    scalar::y210_to_hsv_row::<true>(packed, h_out, s_out, v_out, width, matrix, full_range)
  );
}

#[cfg(all(test, feature = "std"))]
mod tests {
  //! Smoke tests for the public Y210 dispatchers. Walker / kernel
  ...
  #[test]
  fn y210_dispatchers_route_with_simd_false() {
