...
>(
  ...
  unsafe {
    ...
    // Per-u16-lane byte-swap mask for the 256-bit half-width U/V
    // loads. AVX-512 has no public 256-bit BE load helper; we shuffle
    // inline via AVX2's `_mm256_shuffle_epi8`.
    ...
    while x + 32 <= width {
      ...
      // iteration where `x / 2 + 16 == u_half.len()`. BE input is
      // byte-swapped via `load_endian_u16x32::<BE>` for Y and via
      // inline `_mm256_shuffle_epi8` for the half-width U/V loads.
      let y_vec = endian::load_endian_u16x32::<BE>(y.as_ptr().add(x) as *const u8);
...
//
// The AVX-512 twins of the scalar `yuv_*16_to_hsv_row` kernels. Same
// reused-8-bit-RGB-chunk staging as the high-bit (9/10/12/14) HSV
// kernels, over the AVX-512 `yuv_*16_to_rgb_row::<BE>` 8-bit kernel +
// `rgb_to_hsv_row` — byte-identical to
// `rgb_to_hsv_row(yuv_*16_to_rgb_row::<BE>(...))` within the AVX-512
// tier, the same intermediate the existing 16-bit HSV path uses.

/// AVX-512: 16-bit YUV 4:2:0 planar → planar HSV bytes (OpenCV
/// encoding), staged via the AVX-512 [`yuv_420p16_to_rgb_row`] +
...
pub(crate) unsafe fn yuv_420p16_to_hsv_row<const BE: bool>(
  ...
  // forwards to the AVX-512 16-bit 4:2:0 RGB kernel under the same
...
/// AVX-512: 16-bit YUV 4:4:4 planar → planar HSV bytes, staged via the
/// AVX-512 [`yuv_444p16_to_rgb_row`] + [`rgb_to_hsv_row`]. `BE` selects
...
pub(crate) unsafe fn yuv_444p16_to_hsv_row<const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the AVX-512 16-bit 4:4:4 RGB kernel under the same
  // contract (its own scalar tail covers small n).
  unsafe {
    yuv_to_hsv_via_rgb_chunks(h_out, s_out, v_out, width, |offset, n, rgb| {
