...
//
// PR #104 follow-up: pre-fix, the AVX-512 X2 10-bit kernels gated their
// SIMD body on `if !BE` and silently fell through to scalar for BE input.
// With the BE-aware load (`x2_load_endian_u32x4::<BE>`) in `x86_common`,
// BE now exercises the real SIMD path. Widths cross the 64-pixel SIMD
// boundary plus tail.
