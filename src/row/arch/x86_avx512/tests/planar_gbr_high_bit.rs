//! AVX-512 (F+BW) parity tests for the high-bit-depth planar-GBR kernels (Tier 10b).
//!
//! Covers BITS=10 and BITS=16 at widths [1, 7, 8, 16, 17, 32, 33, 64, 128, 130].
//! Each test early-returns when AVX-512BW is not available (sanitizer/Miri safety).
...
// ---- Upper-bits masking: AVX-512 must match scalar for dirty inputs -------
...
// ---- BE parity: AVX-512<BITS, true> on BE-encoded storage must match -------
// AVX-512<BITS, false> on LE-encoded storage. Each plane is built from the ----
