...
fn check_rgb(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v210→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v210→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v210→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v210→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn avx512_v210_rgb_matches_scalar_all_matrices() {
  ...
  for m in [
    ...
    for full in [true, false] {
      // Width 24 = one main-loop quad (no tail).
      check_rgb(24, m, full);
      check_rgba(24, m, full);
      check_rgb_u16(24, m, full);
      check_rgba_u16(24, m, full);
    }
  }
}
...
fn avx512_v210_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn avx512_v210_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
/// U[k] = 2k + 1 (one chroma sample per 2 pixels at 4:2:2) for eight
/// V210 6-pixel groups (48 px = two AVX-512 main-loop quads). V = 512
/// (neutral midpoint, bias-subtracted = 0).
///
/// Asserts:
/// - `luma_u16_row` output = [1..=48] (Y values direct, no shift)
...
fn avx512_v210_lane_order_per_pixel_y_and_u() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
