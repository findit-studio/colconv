...
/// Builds a deterministic pseudo-random V30X buffer with `n` pixels.
...
fn check_rgb(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v30x→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v30x→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v30x→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 v30x→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_luma(width: usize) {
  ...
  assert_eq!(s, k, "AVX-512 v30x→luma diverges (width={width})");
}
...
fn check_luma_u16(width: usize) {
  ...
  assert_eq!(s, k, "AVX-512 v30x→luma u16 diverges (width={width})");
}
...
fn avx512_v30x_rgb_matches_scalar_all_matrices() {
...
fn avx512_v30x_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn avx512_v30x_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn avx512_v30x_lane_order_per_pixel_y_and_u() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
  ...
  assert_eq!(luma, expected_luma, "avx512 v30x luma reorder bug");
  ...
  assert_eq!(
    simd_rgb, scalar_rgb,
    "avx512 v30x SIMD vs scalar diverges — lane-order bug"
  );
}
