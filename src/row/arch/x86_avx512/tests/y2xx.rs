...
///
/// AVX-512 threshold: 32 px/iter. W=64 covers exactly 2 full SIMD iterations.
...
fn avx512_y2xx_lane_order_per_pixel_y_and_u_bits10() {
...
fn avx512_y2xx_lane_order_per_pixel_y_and_u_bits12() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn check_rgb<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 y2xx<{BITS}>→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 y2xx<{BITS}>→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 y2xx<{BITS}>→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 y2xx<{BITS}>→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_luma_u16<const BITS: u32>(width: usize) {
  ...
  assert_eq!(
    s, k,
    "AVX-512 y2xx<{BITS}>→luma u16 diverges (width={width})"
  );
}
...
fn avx512_y210_rgb_matches_scalar_all_matrices() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn avx512_y210_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
  }
  // 32 = AVX-512 natural block; 1952 = 32 × 61 covers tail.
  for w in [32usize, 34, 62, 64, 66, 128, 1920, 1952] {
...
fn avx512_y210_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
...
fn avx512_y212_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx512f")
    || !std::arch::is_x86_feature_detected!("avx512bw")
  {
    return;
  ...
  for w in [32usize, 34, 62, 64, 66, 128, 1920, 1952] {
    ...
    assert_eq!(
      s_u16, k_u16,
      "AVX-512 y2xx<12>→RGBA u16 diverges (width={w})"
    );
