...
//
// The SIMD twin of the scalar `xv36_to_hsv_row` kernel. Rather than
// re-derive an HSV-specific register pipeline, it fills a small fixed
// reused **8-bit** 8-bit RGB scratch (one `HSV_CHUNK`-pixel chunk at a time)
// using the EXISTING xv36_to_rgb_or_rgba_row::<false, BE> kernel of this file — so the chunk
...
// `rgb_to_hsv_row(xv36_to_rgb_or_rgba_row::<false, BE>(...))` within this SIMD tier — the same 8-bit RGB intermediate the existing XV36 HSV path
// uses with no source-width RGB allocation. The
// scalar tail of the underlying RGB kernel handles widths below the SIMD
// block, so no separate tail is needed here. The A slot
...
/// planes. Byte-identical to `rgb_to_hsv_row(xv36_to_rgb_or_rgba_row::<false, BE>(...))` within this SIMD tier, with no
...
/// SIMD: XV36 (packed 4:4:4, 12-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// SIMD [`xv36_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Byte-identical to
/// `rgb_to_hsv_row(xv36_to_rgb_or_rgba_row::<false, BE>(...))` within this SIMD tier. The padding A slot is dropped (HSV is colour-only).
///
/// # Safety
///
...
pub(crate) unsafe fn xv36_to_hsv_row<const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the SIMD XV36 RGB kernel
