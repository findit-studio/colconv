...
//
// Reuses the AVX-512 [`v30x_to_rgb_or_rgba_row`] to fill a small fixed stack
// 8-bit RGB scratch (one `HSV_CHUNK`-pixel chunk at a time) then runs the
// AVX-512 [`rgb_to_hsv_row`] on the chunk — byte-identical to
// `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the
// AVX-512 tier, with no source-width RGB allocation. The driver is local
...
/// Shared AVX-512 driver: walks `width` in `HSV_CHUNK`-pixel chunks, fills a
/// small reused stack RGB scratch via `fill_rgb` (the existing AVX-512
/// V30X RGB kernel), then runs the AVX-512 [`rgb_to_hsv_row`] on that
...
/// # Safety
///
/// AVX-512 must be available, and `fill_rgb` must uphold the underlying RGB
...
unsafe fn v30x_hsv_via_rgb_chunks(
  ...
  while offset < width {
    ...
    // SAFETY: AVX-512 verified by the wrapper's `#[target_feature]`; the chunk
...
/// AVX-512: V30X (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// AVX-512 [`v30x_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Byte-identical
/// to `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the AVX-512 tier.
///
/// # Safety
///
...
pub(crate) unsafe fn v30x_to_hsv_row(
  ...
  // forwards the per-chunk sub-slices to the AVX-512 V30X RGB kernel under
