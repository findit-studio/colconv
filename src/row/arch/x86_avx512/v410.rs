...
//
// Reuses the AVX-512 [`v410_to_rgb_or_rgba_row`] to fill a small fixed stack
...
/// V410 RGB kernel), then runs the AVX-512 [`rgb_to_hsv_row`] on that
...
/// AVX-512: V410 (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// AVX-512 [`v410_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Const-generic over `BE`. Byte-identical
/// to `rgb_to_hsv_row(v410_to_rgb_or_rgba_row::<false, BE>(...))` within the AVX-512 tier.
///
/// # Safety
///
...
pub(crate) unsafe fn v410_to_hsv_row<const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the AVX-512 V410 RGB kernel under
