//! NEON V410 (packed YUV 4:4:4, 10-bit) kernels.
//!
//! ## Layout
//!
...
//!
//! Load one `uint32x4_t` → three `AND+shift` ops extract U / Y / V
...
pub(crate) unsafe fn v410_to_rgb_or_rgba_row<const ALPHA: bool, const BE: bool>(
  ...
  unsafe {
    ...
    while x + 4 <= width {
      ...
      let v_u32 = vandq_u32(vshrq_n_u32::<20>(words), mask);
      ...
      // V410 is 4:4:4 — no chroma duplication needed; each pixel has its own
...
//
// Reuses the NEON [`v410_to_rgb_or_rgba_row`] to fill a small fixed stack
...
// (mirroring `xv36_hsv_via_rgb_chunks`), gated `yuv-444-packed` with the
...
/// small reused stack RGB scratch via `fill_rgb` (the existing NEON V410
...
/// NEON: V410 (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV encoding),
/// staged via the reused-8-bit-RGB-chunk pattern over the NEON
/// [`v410_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Const-generic over
...
pub(crate) unsafe fn v410_to_hsv_row<const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the NEON V410 RGB kernel under the
