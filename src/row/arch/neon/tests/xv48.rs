...
fn check_rgb<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "NEON xv48<ALPHA={ALPHA}>→{} diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_rgb_u16<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "NEON xv48<ALPHA={ALPHA}>→{} u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
    if ALPHA { "RGBA" } else { "RGB" }
  );
}
...
fn check_luma(width: usize) {
  ...
  assert_eq!(s, k, "NEON xv48→luma diverges (width={width})");
}
...
fn check_luma_u16(width: usize) {
  ...
  assert_eq!(s, k, "NEON xv48→luma u16 diverges (width={width})");
}

#[test]
#[cfg_attr(miri, ignore = "NEON SIMD intrinsics unsupported by Miri")]
fn neon_xv48_rgb_matches_scalar_all_matrices() {
  for m in [
    ColorMatrix::Bt601,
    ColorMatrix::Bt709,
...
#[test]
#[cfg_attr(miri, ignore = "NEON SIMD intrinsics unsupported by Miri")]
fn neon_xv48_matches_scalar_widths() {
  for w in [
...
#[test]
#[cfg_attr(miri, ignore = "NEON SIMD intrinsics unsupported by Miri")]
fn neon_xv48_luma_matches_scalar_widths() {
  for w in [
    1usize, 2, 3, 7, 8, 9, 15, 16, 17, 31, 32, 33, 1920, 1921, 1923,
  ] {
    check_luma(w);
...
fn build_xv48_packed_y_n_plus_1_u_2n_plus_1_v_neutral_x_zero(width: usize) -> std::vec::Vec<u16> {
  ...
  for n in 0..width {
    let y = ((n as u16) + 1) << 8; // slot 1: Y high byte = n+1
    let u = ((2 * (n as u16)) + 1) << 8; // slot 0: U high byte = 2n+1
    packed.push(u);
    packed.push(y);
    packed.push(0x8000u16); // slot 2: V neutral
    packed.push(0u16); // slot 3: X padding
  }
  packed
}
...
fn neon_xv48_lane_order_per_pixel_y_and_u() {
  ...
  // Part 1: Luma natural-order at u16 (no shift — full 16-bit Y).
  ...
  assert_eq!(luma_u16, expected_luma, "neon xv48 luma_u16 reorder bug");
  ...
  assert_eq!(
    simd_rgb, scalar_rgb,
    "neon xv48 SIMD vs scalar diverges (u16 RGB) — lane-order bug"
  );
}

/// SIMD-level BE-vs-LE parity test: probes the `bswap_u16x8_if_be<BE>`
/// gate at the SIMD layer.
#[test]
#[cfg_attr(miri, ignore = "NEON SIMD intrinsics unsupported by Miri")]
fn neon_xv48_be_le_simd_parity() {
  for w in [7usize, 8, 17, 33] {
    ...
    for (alpha, bpp) in [(false, 3usize), (true, 4)] {
      ...
      assert_eq!(
        out_le, out_be,
        "neon xv48 BE-vs-LE SIMD parity failed (alpha={alpha}, w={w}) — endian gate broken"
      );
    }
    ...
    for (alpha, bpp) in [(false, 3usize), (true, 4)] {
      ...
      assert_eq!(
        out_le, out_be,
        "neon xv48 BE-vs-LE SIMD parity failed (u16, alpha={alpha}, w={w}) — endian gate broken"
      );
    }
    ...
    // luma u8
    {
      ...
      assert_eq!(
        out_le, out_be,
        "neon xv48 BE-vs-LE SIMD parity failed (luma u8, w={w}) — endian gate broken"
      );
    }
    ...
    // luma u16
    {
      ...
      assert_eq!(
        out_le, out_be,
        "neon xv48 BE-vs-LE SIMD parity failed (luma u16, w={w}) — endian gate broken"
      );
    }
  }
}
