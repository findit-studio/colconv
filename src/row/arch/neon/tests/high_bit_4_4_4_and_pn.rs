...
fn check_planar_u16_neon_rgba_equivalence_n<const BITS: u32>(
  ...
  unsafe {
    yuv_420p_n_to_rgba_u16_row::<BITS, false>(&y, &u, &v, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
...
fn check_pn_u16_neon_rgba_equivalence_n<const BITS: u32>(
  ...
  unsafe {
    p_n_to_rgba_u16_row::<BITS, false>(&y, &uv, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_yuv420p16_u16_neon_rgba_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  let mut rgba_scalar = std::vec![0u16; width * 4];
  let mut rgba_neon = std::vec![0u16; width * 4];
  scalar::yuv_420p16_to_rgba_u16_row::<false>(&y, &u, &v, &mut rgba_scalar, width, matrix, full_range);
  unsafe {
    yuv_420p16_to_rgba_u16_row::<false>(&y, &u, &v, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_p016_u16_neon_rgba_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    p16_to_rgba_u16_row::<false>(&y, &uv, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_yuv444p_n_u8_neon_equivalence<const BITS: u32>(
  ...
  scalar::yuv_444p_n_to_rgb_row::<BITS, false>(&y, &u, &v, &mut rgb_scalar, width, matrix, full_range);
  unsafe {
    yuv_444p_n_to_rgb_row::<BITS, false>(&y, &u, &v, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_yuv444p_n_u16_neon_equivalence<const BITS: u32>(
  ...
  scalar::yuv_444p_n_to_rgb_u16_row::<BITS, false>(&y, &u, &v, &mut rgb_scalar, width, matrix, full_range);
  unsafe {
    yuv_444p_n_to_rgb_u16_row::<BITS, false>(&y, &u, &v, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_yuv444p16_u8_neon_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    yuv_444p16_to_rgb_row::<false>(&y, &u, &v, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_yuv444p16_u16_neon_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  let mut rgb_scalar = std::vec![0u16; width * 3];
  let mut rgb_neon = std::vec![0u16; width * 3];
  scalar::yuv_444p16_to_rgb_u16_row::<false>(&y, &u, &v, &mut rgb_scalar, width, matrix, full_range);
  unsafe {
    yuv_444p16_to_rgb_u16_row::<false>(&y, &u, &v, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_p_n_444_u8_neon_equivalence<const BITS: u32>(
  ...
  unsafe {
    p_n_444_to_rgb_row::<BITS, false>(&y, &uv, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_p_n_444_u16_neon_equivalence<const BITS: u32>(
  ...
  let mut rgb_scalar = std::vec![0u16; width * 3];
  let mut rgb_neon = std::vec![0u16; width * 3];
  scalar::p_n_444_to_rgb_u16_row::<BITS, false>(&y, &uv, &mut rgb_scalar, width, matrix, full_range);
  unsafe {
    p_n_444_to_rgb_u16_row::<BITS, false>(&y, &uv, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_p_n_444_16_u8_neon_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    p_n_444_16_to_rgb_row::<false>(&y, &uv, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_p_n_444_16_u16_neon_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    p_n_444_16_to_rgb_u16_row::<false>(&y, &uv, &mut rgb_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgb_scalar, rgb_neon,
...
fn check_yuv444p_n_u8_neon_rgba_equivalence<const BITS: u32>(
  ...
  scalar::yuv_444p_n_to_rgba_row::<BITS, false>(&y, &u, &v, &mut rgba_scalar, width, matrix, full_range);
  unsafe {
    yuv_444p_n_to_rgba_row::<BITS, false>(&y, &u, &v, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_pn_444_u8_neon_rgba_equivalence<const BITS: u32>(
  ...
  unsafe {
    p_n_444_to_rgba_row::<BITS, false>(&y, &uv, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_yuv444p16_u8_neon_rgba_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    yuv_444p16_to_rgba_row::<false>(&y, &u, &v, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
...
fn check_p_n_444_16_u8_neon_rgba_equivalence(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  unsafe {
    p_n_444_16_to_rgba_row::<false>(&y, &uv, &mut rgba_neon, width, matrix, full_range);
  }
  assert_eq!(
    rgba_scalar, rgba_neon,
