...
//
// Reuses the NEON [`v30x_to_rgb_or_rgba_row`] to fill a small fixed stack
// 8-bit RGB scratch (one `HSV_CHUNK`-pixel chunk at a time) then runs the
// NEON [`rgb_to_hsv_row`] on the chunk — byte-identical to
// `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the NEON
// tier, with no source-width RGB allocation. The driver is local (mirroring
// `xv36_hsv_via_rgb_chunks`), gated `yuv-444-packed` with the rest of this
...
/// small reused stack RGB scratch via `fill_rgb` (the existing NEON V30X
...
/// NEON: V30X (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV encoding),
/// staged via the reused-8-bit-RGB-chunk pattern over the NEON
/// [`v30x_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Byte-identical to
/// `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the NEON
/// tier.
///
/// # Safety
...
pub(crate) unsafe fn v30x_to_hsv_row(
  ...
  // forwards the per-chunk sub-slices to the NEON V30X RGB kernel under the
  // same contract (its own scalar tail covers small n).
  unsafe {
