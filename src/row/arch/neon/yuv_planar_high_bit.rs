...
//
// The NEON twins of the scalar `yuv_*p_n_to_hsv_row` kernels
...
// NEON high-bit `yuv_*p_n_to_rgb_row::<BITS, BE>` kernel — so the
// chunk filler IS the production 8-bit RGB kernel — then runs the
// NEON `rgb_to_hsv_row` on the chunk. This makes the result
// byte-identical to `rgb_to_hsv_row(yuv_*p_n_to_rgb_row::<BITS,
// BE>(...))` within the NEON tier (the same 8-bit RGB intermediate
...
/// NEON: high-bit-depth YUV 4:2:0 planar → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// NEON [`yuv_420p_n_to_rgb_row`] + [`rgb_to_hsv_row`]. Const-generic
...
/// NEON: high-bit-depth YUV 4:4:4 planar → planar HSV bytes, staged
/// via the NEON [`yuv_444p_n_to_rgb_row`] + [`rgb_to_hsv_row`].
...
pub(crate) unsafe fn yuv_444p_n_to_hsv_row<const BITS: u32, const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the NEON high-bit 4:4:4 RGB kernel under the same
