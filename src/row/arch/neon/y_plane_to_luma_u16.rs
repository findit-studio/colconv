//! NEON `y_plane_to_luma_u16_row` — zero-extends a u8 Y plane to u16.
//!
//! Processes 16 pixels per iteration using `vld1q_u8` + `vmovl_u8` /
//! `vmovl_u8(vget_high_u8)` to produce two `uint16x8_t` vectors, then
//! stores via two `vst1q_u16` calls. Scalar tail delegates to the
...
/// NEON zero-extension: `out[x] = plane[x] as u16` for `x in 0..width`.
///
/// Block size: 16 px / iter.
///
/// # Safety
///
/// NEON must be available (baseline on aarch64). `plane.len() >= width`;
/// `out.len() >= width`.
...
pub(crate) unsafe fn y_plane_to_luma_u16_row(plane: &[u8], out: &mut [u16], width: usize) {
  ...
  unsafe {
    while x + 16 <= width {
      let v = vld1q_u8(plane.as_ptr().add(x));
      let low = vmovl_u8(vget_low_u8(v));
      let high = vmovl_u8(vget_high_u8(v));
      vst1q_u16(out.as_mut_ptr().add(x), low);
      vst1q_u16(out.as_mut_ptr().add(x + 8), high);
      x += 16;
    }
  }
...
mod tests {
  ...
  fn neon_y_plane_to_luma_u16_matches_scalar_widths() {
    for &w in WIDTHS {
