...
fn check_rgb<const ALPHA: bool, const ALPHA_SRC: bool>(
  ...
  assert_eq!(
    s,
    k,
    "SSE4.1 ayuv64<ALPHA={ALPHA}, ALPHA_SRC={ALPHA_SRC}>→{} diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_rgb_u16<const ALPHA: bool, const ALPHA_SRC: bool>(
  ...
  assert_eq!(
    s,
    k,
    "SSE4.1 ayuv64<ALPHA={ALPHA}, ALPHA_SRC={ALPHA_SRC}>→{} u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
    if ALPHA { "RGBA" } else { "RGB" }
  );
}
...
fn sse41_ayuv64_rgb_matches_scalar_all_matrices() {
...
fn sse41_ayuv64_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  for w in [
...
///
/// Encodes Y[n] = n + 1 and A[n] = 2n + 1 for n in 0..16.
/// Uses 16 pixels to cover both:
/// - the u8 SIMD block (16 px/iter, full block)
/// - the u16 SIMD block (8 px/iter, two full blocks)
///
...
///
/// This confirms that A and Y lanes are deinterleaved into the correct SSE4.1
/// channels (slot 0 = A, slot 1 = Y) and neither bleeds into the other.
#[test]
#[cfg_attr(
  miri,
  ignore = "SIMD-dispatched row kernels use intrinsics unsupported by Miri"
)]
fn sse41_ayuv64_lane_order_per_pixel_y_and_a() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
