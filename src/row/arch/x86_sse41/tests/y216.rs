...
fn check_rgb<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "SSE4.1 y216→{} diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_rgb_u16<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "SSE4.1 y216→{} u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
    if ALPHA { "RGBA" } else { "RGB" }
  );
}
...
fn sse41_y216_rgb_matches_scalar_all_matrices() {
...
fn sse41_y216_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
...
fn sse41_y216_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  for w in [2usize, 4, 14, 16, 18, 30, 32, 34, 62, 64, 66, 1920, 1922] {
