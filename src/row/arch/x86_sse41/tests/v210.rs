...
fn check_rgb(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 v210→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 v210→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 v210→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 v210→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn sse41_v210_rgb_matches_scalar_all_matrices() {
...
fn sse41_v210_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
...
fn sse41_v210_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  for w in [
...
/// V210 6-pixel groups (12 px = 2 SSE4.1 main-loop iterations). V = 512
...
fn sse41_v210_lane_order_per_pixel_y_and_u() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  const W: usize = 12;
  ...
  assert_eq!(
    simd_rgb, scalar_rgb,
    "sse4.1 v210 SIMD vs scalar diverges — chroma deinterleave bug"
  );
}
