...
///
/// SSE4.1 threshold: 8 px/iter. W=16 covers exactly 2 full SIMD iterations.
...
fn sse41_y2xx_lane_order_per_pixel_y_and_u_bits10() {
...
fn sse41_y2xx_lane_order_per_pixel_y_and_u_bits12() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
...
fn check_rgb<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 y2xx<{BITS}>→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 y2xx<{BITS}>→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 y2xx<{BITS}>→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16<const BITS: u32>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 y2xx<{BITS}>→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_luma_u16<const BITS: u32>(width: usize) {
  ...
  assert_eq!(
    s, k,
    "SSE4.1 y2xx<{BITS}>→luma u16 diverges (width={width})"
  );
}
...
fn sse41_y210_rgb_matches_scalar_all_matrices() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  for m in [
...
fn sse41_y210_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
...
fn sse41_y210_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
  for w in [2usize, 4, 14, 16, 18, 30, 32, 34, 62, 64, 66, 1920, 1922] {
...
fn sse41_y212_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("sse4.1") {
    return;
  }
