...
/// `_mm_shuffle_epi8`).
/// Gather mask for one channel at byte offset `OFF` within each 4-byte
/// pixel (`OFF, OFF+4, OFF+8, OFF+12` into the low 4 bytes; `-1` zeroes the
/// rest). The const offset makes the mask a compile-time constant.
#[inline]
#[target_feature(enable = "sse4.1")]
unsafe fn chan_mask_sse<const OFF: usize>() -> __m128i {
  let o = OFF as i8;
  _mm_setr_epi8(
    o,
    o + 4,
...
/// Offset-parameterized SSE4.1 deinterleave of 16 four-byte pixels into
/// `(v_vec, u_vec, y_vec, a_vec)` (each `__m128i` of 16 natural-order
...
/// # Safety
///
/// `ptr` must point to at least 64 readable bytes (16 pixels). SSE4.1 must
/// be available.
#[inline]
#[target_feature(enable = "sse4.1")]
unsafe fn deinterleave_packed444_sse<
  const V_OFF: usize,
  const U_OFF: usize,
  const Y_OFF: usize,
  const A_OFF: usize,
>(
  ...
  unsafe {
    ...
    let v_mask = chan_mask_sse::<V_OFF>();
    let u_mask = chan_mask_sse::<U_OFF>();
    let y_mask = chan_mask_sse::<Y_OFF>();
    let a_mask = chan_mask_sse::<A_OFF>();
...
>(
  ...
  unsafe {
    ...
    let alpha_u8 = _mm_set1_epi8(-1i8); // 0xFF for the forced-opaque path

    let mut x = 0usize;
    while x + 16 <= width {
      // Deinterleave 16 quadruples → V, U, Y, A as u8x16 per the format's
      // byte offsets.
      let (v_u8, u_u8, y_u8, a_u8) =
        deinterleave_packed444_sse::<V_OFF, U_OFF, Y_OFF, A_OFF>(packed.as_ptr().add(x * 4));
...
pub(crate) unsafe fn packed444_to_luma_row<const Y_OFF: usize>(
  ...
  unsafe {
    // Y bytes are at `Y_OFF, Y_OFF+4, Y_OFF+8, Y_OFF+12` per 16-byte chunk.
    let y_mask = chan_mask_sse::<Y_OFF>();
...
/// SSE4.1 VUYA / VUYX u8 luma (Y at offset 2) over [`packed444_to_luma_row`].
///
/// # Safety
///
...
/// SSE4.1 VUYA u16 luma (Y at offset 2) over [`packed444_to_luma_u16_row`].
///
/// # Safety
///
...
>(
  ...
  // forwards the per-chunk sub-slices to the offset-generic SSE RGB kernel
...
// ---- AYUV / UYVA SSE4.1 wrappers --------------------------------------

/// SSE4.1 AYUV (`A=0,Y=1,U=2,V=3`) → packed RGB.
///
/// # Safety
///
/// Same contract as [`packed444_to_rgb_or_rgba_row`].
...
/// SSE4.1 AYUV → packed RGBA (source α at offset 0).
///
/// # Safety
///
/// Same contract as [`packed444_to_rgb_or_rgba_row`].
...
/// SSE4.1 AYUV → u8 luma (Y at offset 1).
///
/// # Safety
///
/// Same contract as [`packed444_to_luma_row`].
...
/// SSE4.1 AYUV → u16 luma (Y at offset 1).
///
/// # Safety
///
/// Same contract as [`packed444_to_luma_u16_row`].
...
/// SSE4.1 UYVA (`U=0,Y=1,V=2,A=3`) → packed RGB.
///
/// # Safety
///
/// Same contract as [`packed444_to_rgb_or_rgba_row`].
...
/// SSE4.1 UYVA → packed RGBA (source α at offset 3).
///
/// # Safety
///
/// Same contract as [`packed444_to_rgb_or_rgba_row`].
...
/// SSE4.1 UYVA → u8 luma (Y at offset 1).
///
/// # Safety
///
/// Same contract as [`packed444_to_luma_row`].
...
/// SSE4.1 UYVA → u16 luma (Y at offset 1).
///
/// # Safety
///
/// Same contract as [`packed444_to_luma_u16_row`].
...
// ---- VYU444 (V=0, Y=1, U=2; 3 bytes per pixel, no alpha) SSE4.1 --------
//
// VYU444 packs three bytes per pixel (`V ‖ Y ‖ U`, 24bpp). The shared
// [`deinterleave_rgb_16`] 3-byte de-interleave (three `_mm_loadu_si128`
// loads + nine `_mm_shuffle_epi8` byte-gathers + `_mm_or_si128`, the same
// pattern bgr24/rgb24 use) splits 16 triples into three channel vectors
// `(c0, c1, c2)` at byte offsets 0/1/2 = `(V, Y, U)`. Those feed the same
...
/// SSE4.1 VYU444 → packed u8 RGB (`ALPHA = false`) or RGBA
...
pub(crate) unsafe fn vyu444_to_rgb_or_rgba_row<const ALPHA: bool>(
  ...
  unsafe {
    ...
    let cbu = _mm_set1_epi32(coeffs.b_u());
    let cbv = _mm_set1_epi32(coeffs.b_v());
    let alpha_u8 = _mm_set1_epi8(-1i8); // 0xFF for the forced-opaque RGBA path

    let mut x = 0usize;
    while x + 16 <= width {
      // 3-byte de-interleave: channels at offsets 0/1/2 → (V, Y, U).
      let (v_u8, y_u8, u_u8) = deinterleave_rgb_16(packed.as_ptr().add(x * 3));
      ...
      let out_ptr = out.as_mut_ptr().add(x * bpp);
      if ALPHA {
        write_rgba_16(r_u8, g_u8, b_u8, alpha_u8, out_ptr);
      } else {
        write_rgb_16(r_u8, g_u8, b_u8, out_ptr);
      }
...
/// SSE4.1 VYU444 → packed RGB (3 bpp).
///
/// # Safety
///
/// `packed.len() >= width * 3`; `rgb_out.len() >= width * 3`. SSE4.1 must
/// be available.
#[inline]
#[target_feature(enable = "sse4.1")]
...
/// SSE4.1 VYU444 → packed RGBA (4 bpp, α forced `0xFF`).
///
/// # Safety
///
/// `packed.len() >= width * 3`; `rgba_out.len() >= width * 4`. SSE4.1 must
/// be available.
#[inline]
#[target_feature(enable = "sse4.1")]
...
/// SSE4.1 VYU444 → planar HSV bytes, staged via the reused-8-bit-RGB-chunk
/// pattern over the SSE4.1 [`vyu444_to_rgb_row`] + [`rgb_to_hsv_row`].
/// Byte-identical to `rgb_to_hsv_row(vyu444_to_rgb_row(...))` within this
/// tier.
///
/// # Safety
///
/// `packed.len() >= width * 3`; each H/S/V plane `>= width`. SSE4.1 must
/// be available.
#[inline]
#[target_feature(enable = "sse4.1")]
...
/// SSE4.1 VYU444 → u8 luma (Y at offset 1, 3-byte stride). The 3-byte
/// de-interleave's channel 1 delivers Y for all 16 pixels.
///
/// # Safety
///
/// `packed.len() >= width * 3`; `luma_out.len() >= width`. SSE4.1 must be
/// available.
#[inline]
#[target_feature(enable = "sse4.1")]
pub(crate) unsafe fn vyu444_to_luma_row(packed: &[u8], luma_out: &mut [u8], width: usize) {
  ...
  unsafe {
    let mut x = 0usize;
    while x + 16 <= width {
      let (_v, y_u8, _u) = deinterleave_rgb_16(packed.as_ptr().add(x * 3));
...
/// SSE4.1 VYU444 → u16 luma (zero-extended Y at offset 1, 3-byte stride).
///
/// # Safety
///
/// `packed.len() >= width * 3`; `out.len() >= width`. SSE4.1 must be
/// available.
#[cfg_attr(not(any(feature = "std", feature = "alloc")), allow(dead_code))]
#[inline]
#[target_feature(enable = "sse4.1")]
pub(crate) unsafe fn vyu444_to_luma_u16_row(packed: &[u8], out: &mut [u16], width: usize) {
  ...
  unsafe {
    ...
    while x + 16 <= width {
      let (_v, y_u8, _u) = deinterleave_rgb_16(packed.as_ptr().add(x * 3));
      let lo_u16 = _mm_cvtepu8_epi16(y_u8);
      let hi_u16 = _mm_cvtepu8_epi16(_mm_srli_si128::<8>(y_u8));
      _mm_storeu_si128(out.as_mut_ptr().add(x).cast(), lo_u16);
      _mm_storeu_si128(out.as_mut_ptr().add(x + 8).cast(), hi_u16);
      x += 16;
    }
