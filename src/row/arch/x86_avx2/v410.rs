//! AVX2 V410 (packed YUV 4:4:4, 10-bit) kernels.
//!
//! ## Layout
//!
...
/// Unpacks 8 V410 u32 words (one `__m256i`) into three i16x16 vectors
/// holding the 10-bit U / Y / V fields in their low 8 lanes (lanes 8..15
/// are zero / don't-care).
///
/// Strategy: one 256-bit load → three `AND + srli_epi32` ops (all in
...
unsafe fn unpack_v410_8px_avx2<const BE: bool>(ptr: *const u32) -> (__m256i, __m256i, __m256i) {
  ...
  unsafe {
    ...
    let u_i32 = _mm256_and_si256(words, mask); // bits [9:0]
    let y_i32 = _mm256_and_si256(_mm256_srli_epi32::<10>(words), mask); // bits [19:10]
    let v_i32 = _mm256_and_si256(_mm256_srli_epi32::<20>(words), mask); // bits [29:20]
...
/// AVX2 V410 → packed u8 RGB or RGBA.
///
...
pub(crate) unsafe fn v410_to_rgb_or_rgba_row<const ALPHA: bool, const BE: bool>(
  ...
  unsafe {
    ...
    while x + 8 <= width {
      ...
      // (V410 is 4:4:4 — no duplication needed). Lanes 8..15 are
      ...
      // Y scale: V410 Y ≤ 1023 fits i16 — use scale_y (not scale_y_u16_avx2).
...
//
// Reuses the AVX2 [`v410_to_rgb_or_rgba_row`] to fill a small fixed stack
// 8-bit RGB scratch (one `HSV_CHUNK`-pixel chunk at a time) then runs the
// AVX2 [`rgb_to_hsv_row`] on the chunk — byte-identical to
// `rgb_to_hsv_row(v410_to_rgb_or_rgba_row::<false, BE>(...))` within the
// AVX2 tier, with no source-width RGB allocation. The driver is local
...
/// small reused stack RGB scratch via `fill_rgb` (the existing AVX2
/// V410 RGB kernel), then runs the AVX2 [`rgb_to_hsv_row`] on that
...
unsafe fn v410_hsv_via_rgb_chunks(
  ...
  while offset < width {
    ...
    // SAFETY: AVX2 verified by the wrapper's `#[target_feature]`; the chunk
...
/// AVX2: V410 (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// AVX2 [`v410_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Const-generic over `BE`. Byte-identical
/// to `rgb_to_hsv_row(v410_to_rgb_or_rgba_row::<false, BE>(...))` within the AVX2 tier.
///
/// # Safety
///
...
pub(crate) unsafe fn v410_to_hsv_row<const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the AVX2 V410 RGB kernel under
...
/// AVX2 V410 → packed native-depth u16 RGB or RGBA (low-bit-packed at
/// 10-bit).
///
...
pub(crate) unsafe fn v410_to_rgb_u16_or_rgba_u16_row<const ALPHA: bool, const BE: bool>(
  ...
  unsafe {
    ...
    while x + 8 <= width {
      ...
      // V410 Y ≤ 1023 fits i16 — use scale_y (not scale_y_u16_avx2).
      let y_scaled = scale_y(y_i16, y_off_v, y_scale_v, rnd_v);
...
/// AVX2 V410 → u8 luma. Y is `(word >> 10) & 0x3FF`, then `>> 2`.
///
...
/// AVX2 V410 → u16 luma (low-bit-packed at 10-bit). Each output `u16`
