...
>(
  ...
  unsafe {
    ...
    while x + 32 <= width {
      ...
      let u_lo_vec = byteswap_u16x16::<BE>(u_lo_vec);
      let v_lo_vec = byteswap_u16x16::<BE>(v_lo_vec);
      let u_hi_vec = byteswap_u16x16::<BE>(u_hi_vec);
      let v_hi_vec = byteswap_u16x16::<BE>(v_hi_vec);
...
//
// The AVX2 twins of the scalar `p_n_444_to_hsv_row` /
...
// `HSV_CHUNK`-pixel chunk at a time) with the EXISTING AVX2
...
// then runs the AVX2 `rgb_to_hsv_row` on the chunk. The result is
// byte-identical to `rgb_to_hsv_row(p_n_444*_to_rgb_row(...))` within the
// AVX2 tier, with no source-width RGB allocation. The scalar tail of each
...
/// small reused stack RGB scratch via `fill_rgb` (the existing AVX2 4:4:4
...
/// within the AVX2 tier, with no source-width RGB allocation.
///
...
/// AVX2: high-bit-packed semi-planar 4:4:4 (P410/P412) → planar HSV bytes
/// (OpenCV encoding), staged via the reused-8-bit-RGB-chunk pattern over
/// the AVX2 [`p_n_444_to_rgb_row`] + [`rgb_to_hsv_row`]. Const-generic
/// over `BITS ∈ {10, 12}` and `BE`. Byte-identical to
/// `rgb_to_hsv_row(p_n_444_to_rgb_row::<BITS, BE>(...))` within the AVX2
/// tier.
///
/// # Safety
///
...
pub(crate) unsafe fn p_n_444_to_hsv_row<const BITS: u32, const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the AVX2 4:4:4 RGB kernel under
...
/// AVX2: P416 (semi-planar 4:4:4, 16-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the AVX2 [`p_n_444_16_to_rgb_row`] +
/// [`rgb_to_hsv_row`]. `BE` selects the source byte order. Byte-identical
/// to `rgb_to_hsv_row(p_n_444_16_to_rgb_row::<BE>(...))` within the AVX2
/// tier.
///
/// # Safety
///
...
pub(crate) unsafe fn p_n_444_16_to_hsv_row<const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the AVX2 P416 RGB kernel under the same contract (its own
