...
>(
  ...
  unsafe {
    ...
    while x + 32 <= width {
      ...
      // BE input is byte-swapped via `load_endian_u16x16::<BE>` first.
...
>(
  ...
  unsafe {
    ...
    while x + 32 <= width {
      ...
      // `_mm256_packs_epi32` narrow stays lossless. BE input is
      // byte-swapped via `load_endian_u16x16::<BE>` first.
      let y_low_i16 = _mm256_and_si256(
        endian::load_endian_u16x16::<BE>(y.as_ptr().add(x) as *const u8),
        mask_v,
      );
...
>(
  ...
  unsafe {
    ...
    while x + 32 <= width {
      ...
      // input is byte-swapped via `load_endian_u16x16::<BE>` first.
...
//
// The AVX2 twins of the scalar `yuv_*p_n_to_hsv_row` kernels
...
// AVX2 high-bit `yuv_*p_n_to_rgb_row::<BITS, BE>` kernel — so the
// chunk filler IS the production 8-bit RGB kernel — then runs the
// AVX2 `rgb_to_hsv_row` on the chunk. This makes the result
// byte-identical to `rgb_to_hsv_row(yuv_*p_n_to_rgb_row::<BITS,
// BE>(...))` within the AVX2 tier (the same 8-bit RGB intermediate
...
/// AVX2: high-bit-depth YUV 4:4:4 planar → planar HSV bytes, staged
/// via the AVX2 [`yuv_444p_n_to_rgb_row`] + [`rgb_to_hsv_row`].
...
pub(crate) unsafe fn yuv_444p_n_to_hsv_row<const BITS: u32, const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the AVX2 high-bit 4:4:4 RGB kernel under the same
...
unsafe fn extract_hb_simd<const BITS: u32, const MSB: bool>(
  ...
  if MSB {
    _mm256_srl_epi16(raw, _mm_cvtsi32_si128((16 - BITS) as i32))
  } else {
    _mm256_and_si256(raw, mask_v)
  }
}
