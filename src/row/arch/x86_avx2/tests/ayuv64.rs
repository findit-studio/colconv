...
fn avx2_ayuv64_rgb_matches_scalar_all_matrices() {
  ...
  for m in [
    ...
    for full in [true, false] {
      // Width 32 = one main-loop iteration of the u8 path (no tail).
      // Also covers two main-loop iterations of the u16 path (16 px each).
      check_rgb::<false, false>(32, m, full); // RGB
      check_rgb::<true, true>(32, m, full); // RGBA + source alpha
      check_rgb_u16::<false, false>(32, m, full); // RGB u16
      check_rgb_u16::<true, true>(32, m, full); // RGBA u16 + source alpha
    }
  }
}
...
fn avx2_ayuv64_matches_scalar_widths() {
  ...
  // Width sweep covers:
  //   - tail-only widths < 32 (no SIMD main loop in u8 path)
  //   - u16 SIMD block boundary at 16 (one main-loop iteration of u16 path)
  //   - u8 SIMD block-boundary 32 (one main-loop iteration of u8 path, no tail)
  //   - partial-block-plus-tail 47/48/49 (one u8 main-loop + 15/16/17-px tail)
...
///
/// Uses 32 pixels to cover both:
/// - the u8 SIMD block (32 px/iter, full block — most stringent)
/// - the u16 SIMD block (16 px/iter, two full blocks)
///
...
///
/// This catches BOTH the Ship 12b bug class (extra `_mm256_permute4x64_epi64`
/// after the unpack cascade scrambles lane order) AND the asymmetric-mask
/// bug class (per-channel shuffle masks gathering bytes from the wrong
/// channel slot). It confirms that A and Y lanes are deinterleaved into
/// the correct AVX2 channel vectors (slot 0 = A, slot 1 = Y) and that
