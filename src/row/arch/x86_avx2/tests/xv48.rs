...
fn check_rgb<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "AVX2 xv48<ALPHA={ALPHA}>→{} diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_rgb_u16<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "AVX2 xv48<ALPHA={ALPHA}>→{} u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
    if ALPHA { "RGBA" } else { "RGB" }
  );
}
...
fn check_luma(width: usize) {
  ...
  assert_eq!(s, k, "AVX2 xv48→luma diverges (width={width})");
}
...
fn check_luma_u16(width: usize) {
  ...
  assert_eq!(s, k, "AVX2 xv48→luma u16 diverges (width={width})");
}
...
fn avx2_xv48_rgb_matches_scalar_all_matrices() {
...
fn avx2_xv48_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [
...
fn avx2_xv48_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [
...
fn avx2_xv48_lane_order_per_pixel_y_and_u() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  const W: usize = 16;
...
fn avx2_xv48_be_le_simd_parity() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
