...
fn check_rgb<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "AVX2 v30x→{} diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_rgb_u16<const ALPHA: bool>(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s,
    k,
    "AVX2 v30x→{} u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})",
...
fn check_luma(width: usize) {
  ...
  assert_eq!(s, k, "AVX2 v30x→luma diverges (width={width})");
}
...
fn check_luma_u16(width: usize) {
  ...
  assert_eq!(s, k, "AVX2 v30x→luma u16 diverges (width={width})");
}
...
fn avx2_v30x_rgb_matches_scalar_all_matrices() {
...
fn avx2_v30x_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
...
fn avx2_v30x_luma_matches_scalar_widths() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [1usize, 2, 3, 4, 5, 6, 7, 8, 9, 15, 16, 17, 1920, 1921, 1923] {
