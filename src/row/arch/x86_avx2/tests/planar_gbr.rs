//! AVX2 parity tests for the planar-GBR kernels (Tier 10).
...
#[test]
fn avx2_gbr_to_rgb_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [1usize, 7, 15, 16, 17, 31, 32, 33, 47, 48, 49, 1920, 1921] {
    ...
    assert_eq!(out_scalar, out_avx, "AVX2 gbr_to_rgb diverges (width={w})");
  }
}

#[test]
fn avx2_gbra_to_rgba_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [1usize, 7, 15, 16, 17, 31, 32, 33, 47, 48, 49, 1920, 1921] {
    ...
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 gbra_to_rgba diverges (width={w})"
    );
  }
}

#[test]
fn avx2_gbr_to_rgba_opaque_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for w in [1usize, 7, 15, 16, 17, 31, 32, 33, 47, 48, 49, 1920, 1921] {
    ...
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 gbr_to_rgba_opaque diverges (width={w})"
    );
  }
}
