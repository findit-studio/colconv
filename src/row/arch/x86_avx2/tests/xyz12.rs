//! AVX2 parity tests for the Tier 12 (DCP / Xyz12) kernels.
//!
//! Every test asserts byte-identical output against the scalar
//! reference (0-ULP parity contract) and early-returns when AVX2 is
//! unavailable on the host (sanitizer / CI safety).

use super::super::*;
use crate::{
  DcpTargetGamut,
  row::arch::x86_avx2::xyz12::{
...
fn avx2_xyz12_to_rgb_matches_scalar() {
  ...
  for &w in WIDTHS {
    for gamut in [
      ...
      assert_eq!(
        out_scalar, out_avx,
        "AVX2 xyz12_to_rgb diverges (w={w}, gamut={gamut:?})"
      );
    }
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_dirty_input_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    unsafe {
      xyz12_to_rgb_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::DciP3);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgb dirty-input diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_be_matches_le() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    assert_eq!(out_le, out_be, "AVX2 xyz12_to_rgb BE/LE mismatch (w={w})");
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgba_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    unsafe {
      xyz12_to_rgba_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::Rec2020);
    }
    assert_eq!(out_scalar, out_avx, "AVX2 xyz12_to_rgba diverges (w={w})");
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_in_register_store_parity() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in &[16usize, 32] {
    for gamut in [
      ...
      assert_eq!(
        out_scalar, out_avx,
        "AVX2 xyz12_to_rgb in-register store parity (w={w}, gamut={gamut:?})"
      );
    }
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgba_in_register_store_parity() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in &[16usize, 32] {
    for gamut in [
      ...
      assert_eq!(
        out_scalar, out_avx,
        "AVX2 xyz12_to_rgba in-register store parity (w={w}, gamut={gamut:?})"
      );
    }
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_u16_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    unsafe {
      xyz12_to_rgb_u16_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::DciP3);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgb_u16 diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgba_u16_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    unsafe {
      xyz12_to_rgba_u16_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::Rec709);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgba_u16 diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_f32_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    let mut out_avx = std::vec![0.0_f32; w * 3];
    scalar::xyz12::xyz12_to_rgb_f32_row::<false>(&xyz, &mut out_scalar, w, DcpTargetGamut::Rec2020);
    unsafe {
      xyz12_to_rgb_f32_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::Rec2020);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgb_f32 diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_xyz_f32_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    unsafe {
      xyz12_to_xyz_f32_row::<false>(&xyz, &mut out_avx, w);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_xyz_f32 diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgb_f16_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    let mut out_avx = std::vec![zero_f16; w * 3];
    scalar::xyz12::xyz12_to_rgb_f16_row::<false>(&xyz, &mut out_scalar, w, DcpTargetGamut::DciP3);
    unsafe {
      xyz12_to_rgb_f16_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::DciP3);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgb_f16 diverges (w={w})"
    );
  }
}
...
#[test]
#[cfg_attr(miri, ignore = "x86 SIMD intrinsics unsupported by Miri")]
fn avx2_xyz12_to_rgba_f16_matches_scalar() {
  if !std::arch::is_x86_feature_detected!("avx2") {
    return;
  }
  for &w in WIDTHS {
    ...
    let mut out_avx = std::vec![zero_f16; w * 4];
    scalar::xyz12::xyz12_to_rgba_f16_row::<false>(&xyz, &mut out_scalar, w, DcpTargetGamut::Rec709);
    unsafe {
      xyz12_to_rgba_f16_row::<false>(&xyz, &mut out_avx, w, DcpTargetGamut::Rec709);
    }
    assert_eq!(
      out_scalar, out_avx,
      "AVX2 xyz12_to_rgba_f16 diverges (w={w})"
    );
  }
}
