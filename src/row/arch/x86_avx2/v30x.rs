...
//
// Reuses the AVX2 [`v30x_to_rgb_or_rgba_row`] to fill a small fixed stack
...
/// V30X RGB kernel), then runs the AVX2 [`rgb_to_hsv_row`] on that
...
/// AVX2: V30X (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// AVX2 [`v30x_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Byte-identical
/// to `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the AVX2 tier.
///
/// # Safety
///
...
pub(crate) unsafe fn v30x_to_hsv_row(
  ...
  // forwards the per-chunk sub-slices to the AVX2 V30X RGB kernel under
