...
//
// The WASM simd128 twins of the scalar `yuv_*p_n_to_hsv_row` kernels
...
// WASM simd128 high-bit `yuv_*p_n_to_rgb_row::<BITS, BE>` kernel — so the
// chunk filler IS the production 8-bit RGB kernel — then runs the
// WASM simd128 `rgb_to_hsv_row` on the chunk. This makes the result
// byte-identical to `rgb_to_hsv_row(yuv_*p_n_to_rgb_row::<BITS,
// BE>(...))` within the WASM simd128 tier (the same 8-bit RGB intermediate
...
pub(crate) unsafe fn yuv_420p_n_to_hsv_row<const BITS: u32, const BE: bool>(
  ...
  // forwards the per-chunk sub-slices to the WASM simd128 high-bit 4:2:0 RGB
...
/// WASM simd128: high-bit-depth YUV 4:4:4 planar → planar HSV bytes, staged
/// via the WASM simd128 [`yuv_444p_n_to_rgb_row`] + [`rgb_to_hsv_row`].
...
pub(crate) unsafe fn yuv_444p_n_to_hsv_row<const BITS: u32, const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the WASM simd128 high-bit 4:4:4 RGB kernel under the same
...
#[inline]
#[target_feature(enable = "simd128")]
unsafe fn extract_hb_simd<const BITS: u32, const MSB: bool>(raw: v128, mask_v: v128) -> v128 {
  if MSB {
    u16x8_shr(raw, (16 - BITS) as u32)
  } else {
    v128_and(raw, mask_v)
  }
}
