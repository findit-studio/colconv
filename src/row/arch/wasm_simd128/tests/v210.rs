...
fn check_rgb(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "simd128 v210→RGB diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "simd128 v210→RGBA diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgb_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "simd128 v210→RGB u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn check_rgba_u16(width: usize, matrix: ColorMatrix, full_range: bool) {
  ...
  assert_eq!(
    s, k,
    "simd128 v210→RGBA u16 diverges (width={width}, matrix={matrix:?}, full_range={full_range})"
  );
}
...
fn wasm_simd128_v210_rgb_matches_scalar_all_matrices() {
  for m in [
    ColorMatrix::Bt601,
    ColorMatrix::Bt709,
...
fn wasm_simd128_v210_matches_scalar_widths() {
...
fn wasm_simd128_v210_luma_matches_scalar_widths() {
  for w in [
    2usize, 4, 6, 8, 10, 12, 14, 18, 24, 30, 1280, 1920, 1922, 1926,
  ] {
    check_luma(w);
...
/// V210 6-pixel groups (12 px = 2 wasm_simd128 main-loop iterations). V = 512
...
fn wasm_simd128_v210_lane_order_per_pixel_y_and_u() {
  const W: usize = 12;
  ...
  assert_eq!(
    ...
    "wasm_simd128 v210 luma reorder bug"
  );
  ...
  assert_eq!(
    simd_rgb, scalar_rgb,
    "wasm_simd128 v210 SIMD vs scalar diverges — chroma deinterleave bug"
  );
}
