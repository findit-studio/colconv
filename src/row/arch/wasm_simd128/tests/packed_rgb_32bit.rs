...
fn pseudo_random_u32_4(n: usize, seed: u64) -> std::vec::Vec<u32> {
  let mut v = std::vec::Vec::with_capacity(n);
  let mut s = seed;
  for _ in 0..n {
    s = s
...
macro_rules! wasm_rgba128_parity {
  ($name:ident, $simd:ident, $scalar:ident, $out_ty:ty, $out_pp:expr, $be:literal) => {
    #[cfg(target_feature = "simd128")]
    #[test]
    fn $name() {
      for &w in widths() {
        let src = pseudo_random_u32_4(w * 4, 0x0BAD_F00D_CAFE_1234 ^ ($out_pp as u64) ^ ($be as u64));
        let mut scalar_out = std::vec![0 as $out_ty; w * $out_pp];
        let mut simd_out = std::vec![0 as $out_ty; w * $out_pp];
        scalar::$scalar::<$be>(&src, &mut scalar_out, w);
        unsafe { $simd::<$be>(&src, &mut simd_out, w) };
...
wasm_rgba128_parity!(
  wasm_rgba128_to_rgb_le,
  wasm_rgba128_to_rgb_row,
  rgba128_to_rgb_row,
  u8,
  3,
  false
...
wasm_rgba128_parity!(
  wasm_rgba128_to_rgba_le,
  wasm_rgba128_to_rgba_row,
  rgba128_to_rgba_row,
  u8,
  4,
  false
...
wasm_rgba128_parity!(
  wasm_rgba128_to_rgb_u16_le,
  wasm_rgba128_to_rgb_u16_row,
...
wasm_rgba128_parity!(
  wasm_rgba128_to_rgba_u16_le,
  wasm_rgba128_to_rgba_u16_row,
