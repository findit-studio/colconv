...
// ===== VYU444 (3-byte, no alpha) wasm-simd128 parity ===================
...
/// Same-tier HSV reference: stage RGB via the wasm VYU444 RGB kernel, then
/// quantize with the wasm `rgb_to_hsv_row`. The crate's HSV contract is a
/// per-tier identity — `vyu444_to_hsv_row` equals
/// `rgb_to_hsv_row(vyu444_to_rgb_row(...))` *within* the SIMD tier, not
...
/// VYU444 wasm parity against scalar: RGB / RGBA (α forced `0xFF`) / luma
  ...
  for w in [
    ...
    for m in [
      ...
      for fr in [false, true] {
        ...
        assert_eq!(s, k, "wasm VYU444 RGB (w={w}, m={m:?}, fr={fr})");
        ...
        assert_eq!(s4, k4, "wasm VYU444 RGBA (w={w}, m={m:?}, fr={fr})");
      }
    }
    ...
    assert_eq!(sl, kl, "wasm VYU444 luma (w={w})");
...
fn wasm_simd128_vyu444_lane_order_y() {
  const W: usize = 32;
  ...
  assert_eq!(luma, expected, "wasm vyu444 luma reorder bug");
}
