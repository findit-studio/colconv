//! wasm-simd128 V30X (packed YUV 4:4:4, 10-bit) kernels.
//!
//! ## Layout
//!
...
//!
//! Load one `v128` = 4 × u32 lanes. Three `shift + AND` ops extract U /
...
/// wasm-simd128 V30X → packed u8 RGB or RGBA.
///
...
pub(crate) unsafe fn v30x_to_rgb_or_rgba_row<const ALPHA: bool>(
  ...
  unsafe {
    ...
    while x + 4 <= width {
      ...
      let u_i32 = v128_and(u32x4_shr(words, 2), mask);
      let y_i32 = v128_and(u32x4_shr(words, 12), mask);
      let v_i32 = v128_and(u32x4_shr(words, 22), mask);
      ...
      // V30X is 4:4:4 — no chroma duplication needed; each pixel has its
...
// ---- V30X → HSV (reused 8-bit RGB chunk) -------------------------------
//
// Reuses the WASM simd128 [`v30x_to_rgb_or_rgba_row`] to fill a small fixed stack
...
// `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the
...
// file; only `rgb_to_hsv_row` (ungated) is shared. V30X is host-native u32
// (no `BE` generic).
...
/// V30X RGB kernel), then runs the WASM simd128 [`rgb_to_hsv_row`] on that
...
/// `v_out` must be `>= width`.
#[inline]
unsafe fn v30x_hsv_via_rgb_chunks(
  h_out: &mut [u8],
  s_out: &mut [u8],
  v_out: &mut [u8],
  width: usize,
...
/// WASM simd128: V30X (packed 4:4:4, 10-bit) → planar HSV bytes (OpenCV
/// encoding), staged via the reused-8-bit-RGB-chunk pattern over the
/// WASM simd128 [`v30x_to_rgb_or_rgba_row`] + [`rgb_to_hsv_row`]. Byte-identical
/// to `rgb_to_hsv_row(v30x_to_rgb_or_rgba_row::<false>(...))` within the WASM simd128 tier.
///
/// # Safety
///
...
#[inline]
#[target_feature(enable = "simd128")]
pub(crate) unsafe fn v30x_to_hsv_row(
  packed: &[u32],
  h_out: &mut [u8],
  s_out: &mut [u8],
  v_out: &mut [u8],
  ...
  // forwards the per-chunk sub-slices to the WASM simd128 V30X RGB kernel under
  // the same contract (its own scalar tail covers small n).
  unsafe {
    v30x_hsv_via_rgb_chunks(h_out, s_out, v_out, width, |offset, n, rgb| {
      v30x_to_rgb_or_rgba_row::<false>(&packed[offset..], rgb, n, matrix, full_range);
    });
  }
}
...
/// wasm-simd128 V30X → packed native-depth u16 RGB or RGBA (low-bit-packed
/// at 10-bit).
///
...
/// wasm-simd128 V30X → u8 luma. Y is `(word >> 12) & 0x3FF`, then `>> 2`.
///
...
/// wasm-simd128 V30X → u16 luma (low-bit-packed at 10-bit).
///
