...
//
// The WASM simd128 twins of the scalar `yuv_*16_to_hsv_row` kernels. Same
// reused-8-bit-RGB-chunk staging as the high-bit (9/10/12/14) HSV
// kernels, over the WASM simd128 `yuv_*16_to_rgb_row::<BE>` 8-bit kernel +
// `rgb_to_hsv_row` — byte-identical to
// `rgb_to_hsv_row(yuv_*16_to_rgb_row::<BE>(...))` within the WASM simd128
// tier, the same intermediate the existing 16-bit HSV path uses.

/// WASM simd128: 16-bit YUV 4:2:0 planar → planar HSV bytes (OpenCV
/// encoding), staged via the WASM simd128 [`yuv_420p16_to_rgb_row`] +
...
pub(crate) unsafe fn yuv_420p16_to_hsv_row<const BE: bool>(
  ...
  // forwards to the WASM simd128 16-bit 4:2:0 RGB kernel under the same
...
/// WASM simd128: 16-bit YUV 4:4:4 planar → planar HSV bytes, staged via the
/// WASM simd128 [`yuv_444p16_to_rgb_row`] + [`rgb_to_hsv_row`]. `BE` selects
...
pub(crate) unsafe fn yuv_444p16_to_hsv_row<const BE: bool>(
  ...
  // SAFETY: the feature is the caller's obligation; the chunk filler
  // forwards to the WASM simd128 16-bit 4:4:4 RGB kernel under the same
  // contract (its own scalar tail covers small n).
  unsafe {
    yuv_to_hsv_via_rgb_chunks(h_out, s_out, v_out, width, |offset, n, rgb| {
