...
impl ClSystem {
  ...
  ///
  /// Returns [`Some`] **only** for BT.2020 primaries, the gamut the CL
  /// `YcCbcCrc` derivation is published and verifiable for. The OETF
  /// bit-depth follows the transfer: [`Transfer::Bt2020_12Bit`] selects the
  /// 12-bit `(α, β)`; **every other** transfer (including
  /// [`Transfer::Bt2020_10Bit`], [`Transfer::Unspecified`], and the PQ/HLG
  /// HDR curves, which are *not* the CL camera gamma) takes the 10-bit
  /// constants — the BT.2020-2 default for a 10-bit system.
  ///
  /// Any non-BT.2020 primary set returns [`None`]: the caller must then fall
  /// back to the affine matrix path rather than apply an unverifiable CL
  /// decode. A source tagged `ColorMatrix::ChromaDerivedCl` without BT.2020
  /// primaries is malformed; the affine fallback is the defined,
  /// non-panicking policy (the same shape ICtCp uses for a non-PQ/HLG
  /// transfer).
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub(crate) const fn resolve(primaries: Primaries, transfer: Transfer) -> Option<Self> {
    match primaries {
      Primaries::Bt2020 => Some(Self {
        is_12_bit: matches!(transfer, Transfer::Bt2020_12Bit),
      }),
      _ => None,
    }
  }
