...
fn rgbf16_frame_try_new_accepts_valid_tight() {
  ...
  Rgbf16Frame::try_new(&buf, 16, 4, 48).expect("valid");
}

#[test]
fn rgbf16_frame_try_new_accepts_oversized_stride() {
  let buf = std::vec![half::f16::ZERO; 64 * 4];
  Rgbf16Frame::try_new(&buf, 16, 4, 64).expect("padded stride is valid");
}
...
fn rgbf16_frame_try_new_accepts_hdr_and_negative_values() {
  ...
  Rgbf16Frame::try_new(&buf, 16, 4, 48).expect("HDR values allowed");
  let neg = std::vec![half::f16::from_f32(-1.0); 16 * 4 * 3];
  Rgbf16Frame::try_new(&neg, 16, 4, 48).expect("negative values allowed");
}
...
fn rgbf16_frame_try_new_rejects_zero_dimension() {
  let buf = std::vec![half::f16::ZERO; 16 * 4 * 3];
  assert!(matches!(
    Rgbf16Frame::try_new(&buf, 0, 4, 48),
  ...
  assert!(matches!(
    Rgbf16Frame::try_new(&buf, 16, 0, 48),
    Err(Rgbf16FrameError::ZeroDimension {
      width: 16,
      height: 0
    })
...
fn rgbf16_frame_try_new_rejects_stride_too_small() {
  let buf = std::vec![half::f16::ZERO; 16 * 4 * 3];
  assert!(matches!(
    Rgbf16Frame::try_new(&buf, 16, 4, 47),
...
fn rgbf16_frame_try_new_rejects_short_plane() {
  ...
  assert!(matches!(
    Rgbf16Frame::try_new(&small, 16, 4, 48),
...
fn rgbf16_frame_try_new_rejects_width_overflow() {
  ...
  assert!(matches!(
    Rgbf16Frame::try_new(&buf, too_big, 1, u32::MAX),
...
fn rgbf16_frame_try_new_rejects_geometry_overflow() {
  ...
  let res = Rgbf16Frame::try_new(&buf, width, height, stride);
  assert!(
    matches!(
      res,
...
fn rgbf16_frame_new_panics_on_invalid() {
  ...
  let _ = Rgbf16Frame::new(&buf, 16, 4, 48);
}
...
fn rgbf16_frame_accessors_round_trip() {
  ...
  let frame = Rgbf16Frame::try_new(&buf, 8, 2, 24).expect("valid");
