...
use crate::{ColorMatrix, PixelSink, SourceFormat, frame::Yuv444p10Frame, sealed::Sealed};

/// Zero‑sized marker for the YUV 4:4:4 **10‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuv444p10;

impl Sealed for Yuv444p10 {}
impl SourceFormat for Yuv444p10 {}

/// One output row of a [`Yuv444p10`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuv444p10Row<'a> {
  y: &'a [u16],
  u: &'a [u16],
  v: &'a [u16],
  row: usize,
...
/// Sinks that consume [`Yuv444p10Row`].
pub trait Yuv444p10Sink: for<'a> PixelSink<Input<'a> = Yuv444p10Row<'a>> {}

/// Walks a [`Yuv444p10Frame`] row by row into the sink.
pub fn yuv444p10_to<S: Yuv444p10Sink>(
  src: &Yuv444p10Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  ...
  for row in 0..h {
    ...
    sink.process(Yuv444p10Row::new(y, u, v, row, matrix, full_range))?;
  }
  Ok(())
}
