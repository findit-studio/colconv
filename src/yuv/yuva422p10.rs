...
use crate::{
  ...
  frame::{Yuva422p10Frame, Yuva422pFrame16},
  sealed::Sealed,
};

/// Zero‑sized marker for the YUVA 4:2:2 **10‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuva422p10;

impl Sealed for Yuva422p10 {}
impl SourceFormat for Yuva422p10 {}

/// One output row of a [`Yuva422p10`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuva422p10Row<'a> {
  y: &'a [u16],
  u_half: &'a [u16],
  v_half: &'a [u16],
  a: &'a [u16],
...
/// Sinks that consume [`Yuva422p10Row`].
pub trait Yuva422p10Sink: for<'a> PixelSink<Input<'a> = Yuva422p10Row<'a>> {}

/// Walks a [`Yuva422p10Frame`] row by row into the sink.
pub fn yuva422p10_to<S: Yuva422p10Sink>(
  src: &Yuva422p10Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  yuva422p10_walker::<10, S>(src, full_range, matrix, sink)
}

#[cfg_attr(not(tarpaulin), inline(always))]
fn yuva422p10_walker<const BITS: u32, S: Yuva422p10Sink>(
  ...
  for row in 0..h {
    ...
    sink.process(Yuva422p10Row::new(
