...
use crate::{
  ...
  frame::{Yuva422p16Frame, Yuva422pFrame16},
  sealed::Sealed,
};

/// Zero‑sized marker for the YUVA 4:2:2 **16‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuva422p16;

impl Sealed for Yuva422p16 {}
impl SourceFormat for Yuva422p16 {}

/// One output row of a [`Yuva422p16`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuva422p16Row<'a> {
  y: &'a [u16],
  u_half: &'a [u16],
  v_half: &'a [u16],
  a: &'a [u16],
...
/// Sinks that consume [`Yuva422p16Row`].
pub trait Yuva422p16Sink: for<'a> PixelSink<Input<'a> = Yuva422p16Row<'a>> {}

/// Walks a [`Yuva422p16Frame`] row by row into the sink.
pub fn yuva422p16_to<S: Yuva422p16Sink>(
  src: &Yuva422p16Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  yuva422p16_walker::<16, S>(src, full_range, matrix, sink)
}

#[cfg_attr(not(tarpaulin), inline(always))]
fn yuva422p16_walker<const BITS: u32, S: Yuva422p16Sink>(
  ...
  for row in 0..h {
    ...
    sink.process(Yuva422p16Row::new(
