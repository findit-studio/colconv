...
use crate::{ColorMatrix, PixelSink, SourceFormat, frame::Yuv422p14Frame, sealed::Sealed};

/// Zero‑sized marker for the YUV 4:2:2 **14‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuv422p14;

impl Sealed for Yuv422p14 {}
impl SourceFormat for Yuv422p14 {}

/// One output row of a [`Yuv422p14`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuv422p14Row<'a> {
  y: &'a [u16],
  u_half: &'a [u16],
  v_half: &'a [u16],
  row: usize,
...
/// Sinks that consume [`Yuv422p14Row`].
pub trait Yuv422p14Sink: for<'a> PixelSink<Input<'a> = Yuv422p14Row<'a>> {}

/// Walks a [`Yuv422p14Frame`] row by row into the sink.
pub fn yuv422p14_to<S: Yuv422p14Sink>(
  src: &Yuv422p14Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
