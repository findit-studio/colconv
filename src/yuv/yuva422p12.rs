...
use crate::{
  ...
  frame::{Yuva422p12Frame, Yuva422pFrame16},
  sealed::Sealed,
};

/// Zero‑sized marker for the YUVA 4:2:2 **12‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuva422p12;

impl Sealed for Yuva422p12 {}
impl SourceFormat for Yuva422p12 {}

/// One output row of a [`Yuva422p12`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuva422p12Row<'a> {
  y: &'a [u16],
  u_half: &'a [u16],
  v_half: &'a [u16],
  a: &'a [u16],
...
/// Sinks that consume [`Yuva422p12Row`].
pub trait Yuva422p12Sink: for<'a> PixelSink<Input<'a> = Yuva422p12Row<'a>> {}

/// Walks a [`Yuva422p12Frame`] row by row into the sink.
pub fn yuva422p12_to<S: Yuva422p12Sink>(
  src: &Yuva422p12Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  yuva422p12_walker::<12, S>(src, full_range, matrix, sink)
}

#[cfg_attr(not(tarpaulin), inline(always))]
fn yuva422p12_walker<const BITS: u32, S: Yuva422p12Sink>(
  ...
  for row in 0..h {
    ...
    sink.process(Yuva422p12Row::new(
