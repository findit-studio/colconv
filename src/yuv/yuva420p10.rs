...
use crate::{
  ...
  frame::{Yuva420p10Frame, Yuva420pFrame16},
  sealed::Sealed,
};

/// Zero‑sized marker for the YUVA 4:2:0 **10‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuva420p10;

impl Sealed for Yuva420p10 {}
impl SourceFormat for Yuva420p10 {}

/// One output row of a [`Yuva420p10`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuva420p10Row<'a> {
  y: &'a [u16],
  u_half: &'a [u16],
  v_half: &'a [u16],
  a: &'a [u16],
...
/// Sinks that consume [`Yuva420p10Row`].
pub trait Yuva420p10Sink: for<'a> PixelSink<Input<'a> = Yuva420p10Row<'a>> {}

/// Walks a [`Yuva420p10Frame`] row by row into the sink.
pub fn yuva420p10_to<S: Yuva420p10Sink>(
  src: &Yuva420p10Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  yuva420p10_walker::<10, S>(src, full_range, matrix, sink)
}

#[cfg_attr(not(tarpaulin), inline(always))]
fn yuva420p10_walker<const BITS: u32, S: Yuva420p10Sink>(
  ...
  for row in 0..h {
    ...
    sink.process(Yuva420p10Row::new(
