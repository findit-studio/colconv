//! P212 — semi‑planar 4:2:2, 12‑bit, high‑bit‑packed
//! (`AV_PIX_FMT_P212LE`, FFmpeg 5.0+).
//!
//! Same layout as [`super::P210`] but with 12 active bits in the high
//! 12 positions of each `u16` (low 4 bits zero). Per-row kernel reuses
//! the 4:2:0 `p_n_to_rgb_*<12>` family verbatim; only the walker
//! reads chroma row `r` instead of `r / 2` (4:2:2 vs 4:2:0).

use crate::{ColorMatrix, PixelSink, SourceFormat, frame::P212Frame, sealed::Sealed};

/// Zero‑sized marker for the P212 source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct P212;

impl Sealed for P212 {}
impl SourceFormat for P212 {}

/// One output row of a P212 source handed to a [`P212Sink`].
#[derive(Debug, Clone, Copy)]
pub struct P212Row<'a> {
  y: &'a [u16],
  uv_half: &'a [u16],
  row: usize,
  matrix: ColorMatrix,
  full_range: bool,
}

impl<'a> P212Row<'a> {
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub(crate) fn new(
    y: &'a [u16],
    uv_half: &'a [u16],
    row: usize,
    matrix: ColorMatrix,
    full_range: bool,
  ) -> Self {
    Self {
      y,
      uv_half,
      row,
      matrix,
      full_range,
    }
  }
  /// Full‑width Y row — `width` `u16` samples, high‑bit‑packed (12 bits).
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub fn y(&self) -> &'a [u16] {
    self.y
  }
  /// Half-width interleaved UV row — `width` `u16` elements.
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub fn uv_half(&self) -> &'a [u16] {
    self.uv_half
  }
  /// Output row index.
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub const fn row(&self) -> usize {
    self.row
  }
  /// YUV → RGB matrix.
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub const fn matrix(&self) -> ColorMatrix {
    self.matrix
  }
  /// Full-range flag.
  #[cfg_attr(not(tarpaulin), inline(always))]
  pub const fn full_range(&self) -> bool {
    self.full_range
  }
}

/// Sinks that consume [`P212Row`].
pub trait P212Sink: for<'a> PixelSink<Input<'a> = P212Row<'a>> {}

/// Walks a [`P212Frame`] row by row into the sink.
pub fn p212_to<S: P212Sink>(
  src: &P212Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  sink.begin_frame(src.width(), src.height())?;

  let w = src.width() as usize;
  let h = src.height() as usize;
  let y_stride = src.y_stride() as usize;
  let uv_stride = src.uv_stride() as usize;
  let uv_row_elems = w;

  let y_plane = src.y();
  let uv_plane = src.uv();

  for row in 0..h {
    let y_start = row * y_stride;
    let y = &y_plane[y_start..y_start + w];

    let uv_start = row * uv_stride;
    let uv_half = &uv_plane[uv_start..uv_start + uv_row_elems];

    sink.process(P212Row::new(y, uv_half, row, matrix, full_range))?;
  }
  Ok(())
}
