...
use crate::{ColorMatrix, PixelSink, SourceFormat, frame::Yuva444p12Frame, sealed::Sealed};

/// Zero‑sized marker for the YUVA 4:4:4 **12‑bit** source format.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash)]
pub struct Yuva444p12;

impl Sealed for Yuva444p12 {}
impl SourceFormat for Yuva444p12 {}

/// One output row of a [`Yuva444p12`] source.
#[derive(Debug, Clone, Copy)]
pub struct Yuva444p12Row<'a> {
  y: &'a [u16],
  u: &'a [u16],
  v: &'a [u16],
  a: &'a [u16],
...
/// Sinks that consume [`Yuva444p12Row`].
pub trait Yuva444p12Sink: for<'a> PixelSink<Input<'a> = Yuva444p12Row<'a>> {}

/// Walks a [`Yuva444p12Frame`] row by row into the sink.
pub fn yuva444p12_to<S: Yuva444p12Sink>(
  src: &Yuva444p12Frame<'_>,
  full_range: bool,
  matrix: ColorMatrix,
  sink: &mut S,
) -> Result<(), S::Error> {
  ...
  for row in 0..h {
    ...
    sink.process(Yuva444p12Row::new(y, u, v, a, row, matrix, full_range))?;
  }
  Ok(())
}
