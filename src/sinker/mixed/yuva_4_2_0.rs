...
impl PixelSink for MixedSinker<'_, Yuva420p> {
  ...
  fn process(&mut self, row: Yuva420pRow<'_>) -> Result<(), Self::Error> {
    ...
    if want_rgba {
      ...
      crate::row::dispatch::alpha_extract::copy_alpha_plane_u8(row.a(), rgba_row, w);
    }
...
>(
  ...
  if want_rgb_u16 {
    ...
    if want_rgba_u16 {
      ...
      crate::row::dispatch::alpha_extract::copy_alpha_plane_u16::<BITS>(a_row, rgba_u16_row, w);
    }
  ...
  if want_rgba {
    ...
    let rgba_row = rgba_plane_row_slice(rgba_buf, one_plane_start, one_plane_end, w, h)?;
    expand_rgb_to_rgba_row(rgb_row, rgba_row, w);
    crate::row::dispatch::alpha_extract::copy_alpha_plane_u16_to_u8::<BITS>(a_row, rgba_row, w);
  }
