...
use crate::{
  PixelSink,
  row::{
    expand_rgb_to_rgba_row, expand_rgb_u16_to_rgba_u16_row, rgb_to_hsv_row, y210_to_hsv_row_endian,
...
impl<R, const BE: bool> PixelSink for MixedSinker<'_, Y210<BE>, R> {
  ...
  fn process(&mut self, row: Y210Row<'_>) -> Result<(), Self::Error> {
    ...
    // (HSV-only routes through the direct `y210_to_hsv_row_endian` kernel, which
    ...
    // HSV-without-RGB-or-RGBA goes through the direct `y210_to_hsv_row_endian`
