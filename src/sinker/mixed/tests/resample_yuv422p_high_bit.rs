//! Fused-downscale coverage for the high-bit **planar** 4:2:2 YUV family —
//! `Yuv422p10` / `Yuv422p12` / `Yuv422p14` / `Yuv422p16` (LE + BE wire).
//! Low-packed `u16` Y plane + half-width U / V planes (horizontal chroma
//! upsampling done in-register by the shared 4:2:0 row kernels).
//!
//! These route through [`packed_yuv422_triple_resample`] (the same tail the
//! packed high-bit 4:2:2 family uses), with **three** independent
//! native-precision binnings:
//! - **u8 colour (rgb / rgba / hsv)** bins a converted source-width u8 RGB
//!   row (`yuv420pN_to_rgb_row_endian` — 4:2:0 and 4:2:2 share the per-row
//!   chroma contract: half-width UV, one pair per Y pair).
...
//!   (BITS - 8)`. (Yuv422p exposes no `luma_u16` output.)
//!
//! Colour oracles drive a direct identity sink at source resolution and
//! 2x2-block-mean its output; the luma oracle area-bins the native Y plane
//! then narrows. Uniform-gray + saturated-chroma counterexamples pin the
...
macro_rules! yuv422p_high_bit_resample_suite {
  (
    ...
    mod $mod {
      ...
      /// native codes so every kernel sees real math).
      ...
      fn chroma_upsample_matches_direct() {
        // 4:2:2 horizontal chroma upsampling happens in-register inside the
        // u8 / u16 conversion closures, so a per-pixel-varying chroma frame
        // must still match the direct convert-then-bin (which upsamples the
        // same way at full resolution).
        let (y, u, v) = ramp();
        let mut rgb = vec![0u8; OUT * OUT * 3];
        let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
        {
...
yuv422p_high_bit_resample_suite!(
  yuv422p10,
  Yuv422p10LeFrame,
  Yuv422p10BeFrame,
  Yuv422p10,
