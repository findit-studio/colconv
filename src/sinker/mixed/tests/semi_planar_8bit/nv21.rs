...
fn nv21_with_luma_u16_extracts_y_zero_extended() {
  let width = 64usize;
  let height = 4usize;
  let n = width * height;
  let ch = height / 2;
  ...
  let mut vup = std::vec![0u8; width * ch];
  ...
  let mut sink = MixedSinker::<Nv21>::new(width, height)
  ...
  assert_eq!(luma_out, expected, "Nv21 luma_u16 mismatch");
}
...
// leaves the output frame untouched. Mirrors the planar_8bit / Yuv420p
// siblings. Reuses the crate's RGB-scratch failpoint (`yuva`-gated, so this
// test is too; under `--all-features` both `yuv-semi-planar` and `yuva` are
// on).
#[cfg(feature = "yuva")]
#[test]
#[cfg_attr(
...
fn nv21_rgb_scratch_alloc_failure_leaves_outputs_untouched() {
  ...
  let err = nv21_to(&src, false, ColorMatrix::Bt601, &mut sink).unwrap_err();
  drop(sink);
