...
//! `Yuv422p` centered decode (the strongest catch for a Y / U / V de-interleave
//! bug); SIMD-vs-scalar parity; the `ColorSpec` siting flow; the
//! `ChromaDerivedNcl` matrix-tag-fallback consistency; the preflight-ordering
//! alloc-failure atomicity (luma untouched); and the no-output invariant (no row
//! math / no allocation when nothing is attached, including a
//! 32-bit-offset-overflow geometry).

#[allow(unused_imports)]
pub(super) use super::*;
pub(super) use crate::ChromaLocation;

pub(super) const W: u32 = 16;
pub(super) const H: u32 = 8;

/// Non-flat luma + per-column chroma ramp on a half-width, **full-height**
/// chroma plane (one chroma row per luma row — 4:2:2). The Y ramp makes a Y
/// de-interleave bug observable; distinct adjacent chroma columns make the
/// horizontal phase observable; the `+ r` term keeps rows distinct so a vertical
/// mistake would surface.
pub(super) fn ramp_planes() -> (Vec<u8>, Vec<u8>, Vec<u8>) {
  let w = W as usize;
  let h = H as usize;
  let cw = w / 2;
  ...
  for r in 0..h {
    for x in 0..w {
      y[r * w + x] = (16 + x * 8 + r * 5).min(235) as u8;
    }
  }
...
/// Packs planar Y (full) + half-width U / V into a YUYV422 plane (`[Y0 U Y1 V]`).
pub(super) fn pack_yuyv(y: &[u8], u: &[u8], v: &[u8]) -> Vec<u8> {
  let w = W as usize;
  let h = H as usize;
  let cw = w / 2;
...
/// Packs planar Y (full) + half-width U / V into a UYVY422 plane (`[U Y0 V Y1]`).
pub(super) fn pack_uyvy(y: &[u8], u: &[u8], v: &[u8]) -> Vec<u8> {
  let w = W as usize;
  let h = H as usize;
  let cw = w / 2;
  let mut out = std::vec![0u8; 2 * w * h];
...
pub(super) fn ref_upsample_center_h(c_half: &[u8], width: usize) -> Vec<u8> {
  let half = width / 2;
...
/// each luma row `r` takes chroma row `r` (1:1 — NO vertical subsampling)
/// upsampled with the centered weights. Feeding these to a `Yuv444p` conversion
/// is the end-to-end oracle.
pub(super) fn ref_full_chroma(u422: &[u8], v422: &[u8]) -> (Vec<u8>, Vec<u8>) {
  let w = W as usize;
  let h = H as usize;
  let cw = w / 2;
...
/// Generates the full centered-siting suite for one packed 4:2:2 layout. Both
/// `Yuyv422` and `Uyvy422` get the identical coverage; only the source marker /
/// row / frame / walker and the planar→packed `$pack` differ.
macro_rules! packed_siting_suite {
  ($modname:ident, $Marker:ident, $Row:ident, $Frame:ident, $walker:ident, $pack:ident) => {
    mod $modname {
      use super::*;
      ...
      fn convert_rgb(loc: ChromaLocation, simd: bool) -> Vec<u8> {
        ...
        let packed = $pack(&yp, &up, &vp);
        let src = $Frame::new(&packed, W, H, W * 2);
      ...
      fn default_path_does_not_allocate_chroma_scratch() {
        ...
        let half_len = sink.packed_yuv_u_half.len();
        let y_len = sink.packed_yuv_y_full.len();
        drop(sink);
        assert_eq!(
          chroma_len, 0,
        ...
        assert_eq!(
          y_len, 0,
          "co-sited path must not grow the Y de-interleave scratch"
        );
      }
      ...
      fn center_rgb_matches_upsample_then_444_reference() {
        ...
        assert_eq!(
          ...
          rgb_ref,
          "centered packed RGB must equal upsample-then-4:4:4"
        );
      }
      ...
      fn center_grows_chroma_scratch_to_full_width() {
        ...
        assert_eq!(
          y_len,
          W as usize,
          "centered path stages the de-interleaved full-width Y"
        );
      }
      ...
      fn centered_matches_yuv422p_centered() {
        // De-interleaving the packed row back to planar Y / U / V must
        // reconstruct the SAME planes the planar twin holds, so a centered packed
        // decode is byte-identical to a centered Yuv422p decode — the strongest
        // catch for a Y / U / V swap in the de-interleave. Bt601: the equivalence
        // holds on the shared matrix-tag path (`ChromaDerivedNcl` is separate).
        let (yp, up, vp) = ramp_planes();
        ...
        assert_eq!(
          convert_rgb(ChromaLocation::Center, true),
          rgb422,
          "centered packed must equal centered Yuv422p of the same planes"
        );
      }

      // ---- ChromaDerivedNcl: packed uses the matrix-tag fallback ----
      ...
      fn centered_chroma_derived_ncl_uses_matrix_tag_fallback() {
        // Packed is NOT ChromaDerivedNcl-primaries-wired. BOTH paths (the default
        // fused kernel AND the centered 4:4:4 kernel) resolve ChromaDerivedNcl via
        // the shared BT.709 matrix-tag fallback (`Coefficients::for_matrix`),
        ...
        assert_eq!(
          ...
          "default ChromaDerivedNcl must resolve via the same matrix-tag fallback"
        );
        assert_ne!(
          decode_cdn(ChromaLocation::Center),
          decode_cdn(ChromaLocation::Left),
      ...
      fn centered_alloc_failure_leaves_outputs_untouched() {
        ...
        // Armed: a centered RGB decode whose chroma-scratch allocation fails must
        // leave EVERY output — luma included — untouched, because the scratch is
        // reserved (fallibly) BEFORE any output row is written.
        let src = $Frame::new(&packed, W, H, W * 2);
        ...
        crate::sinker::mixed::arm_chroma_full_alloc_failure();
      ...
      fn no_output_row_does_not_allocate_chroma_scratch() {
        // A centered-siting sink with NO outputs attached returns before the
        // centered preflight, so neither chroma scratch is ever reserved.
        let (yp, up, vp) = ramp_planes();
        let packed = $pack(&yp, &up, &vp);
        let src = $Frame::new(&packed, W, H, W * 2);
        let mut sink = MixedSinker::<$Marker>::new(W as usize, H as usize)
      ...
      fn no_output_row_large_geometry_does_not_overflow() {
        ...
        let packed = std::vec![128u8; 2 * w]; // one packed row: 2 * w bytes
        let mut sink =
          MixedSinker::<$Marker>::new(w, h).with_chroma_location(ChromaLocation::Center);
        let row = crate::source::$Row::new(&packed, idx, ColorMatrix::Bt601, false);
