//! Alpha-aware fused-downscale coverage for the 8-bit planar 4:2:2 YUV
//! source with a real full-resolution source alpha plane, `Yuva422p`.
//!
//! `Yuva422p` routes through the packed-YUVA tail
//! ([`packed_yuva444_resample`](super::super::packed_yuva444_resample)) at
//! `SRC_BITS = 8`, exactly like `Yuva420p` (the per-row chroma layout —
//! half-width U / V — is identical; only the vertical chroma sampling
//! differs, and the walker owns that). The u8 colour stream bins the
//! converted u8 RGBA row (`yuva420p_to_rgba_row` — horizontal 4:2:2 chroma
//! upsample, real source α), and the native-Y luma stream bins the Y plane
//! directly. Each output is byte-identical to a direct convert-then-bin;
...
use crate::{
  ...
  source::{Yuva422p, Yuva422pRow, yuva422p_to},
};
...
/// Y / A are full-resolution (`SRC * SRC`), U / V are half-width x full-height.
...
/// `Yuva422p` conversion. The oracles bin / premultiply this.
fn direct_rgba(y: &[u8], u: &[u8], v: &[u8], a: &[u8], full_range: bool) -> Vec<u8> {
  let mut rgba = std::vec![0u8; SRC * SRC * 4];
  {
    let mut sink = MixedSinker::<Yuva422p>::new(SRC, SRC)
      .with_rgba(&mut rgba)
      .unwrap();
    yuva422p_to(&frame(y, u, v, a), full_range, M, &mut sink).unwrap();
  }
  rgba
}
...
/// Direct (identity) `Yuva422p` luma / luma_u16 of a binned-Y plane at the
...
fn direct_luma_of_binned_y(binned_y: &[u8], full_range: bool) -> (Vec<u8>, Vec<u16>) {
  let n = binned_y.len();
  let cw = OUT / 2;
  let u = std::vec![128u8; cw * OUT];
  let v = std::vec![128u8; cw * OUT];
  ...
  {
    let mut sink = MixedSinker::<Yuva422p>::new(OUT, OUT)
      .with_luma(&mut luma)
      .unwrap()
      .with_luma_u16(&mut lu16)
      .unwrap();
    yuva422p_to(&src, full_range, M, &mut sink).unwrap();
  }
  (luma, lu16)
}
...
fn yuva422p_straight_rgba_is_block_mean_of_direct() {
  let (y, u, v, a) = planes(0x51A1);
  let mut rgba = std::vec![0u8; OUT * OUT * 4];
  {
    let mut sink =
      MixedSinker::<Yuva422p, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(OUT, OUT))
        .unwrap()
        .with_rgba(&mut rgba)
        .unwrap();
    yuva422p_to(&frame(&y, &u, &v, &a), FR, M, &mut sink).unwrap();
  }
...
fn yuva422p_straight_all_outputs_derive_correctly() {
...
fn yuva422p_premultiplied_matches_premult_bin_unpremult_oracle() {
...
fn yuva422p_premultiplied_transparent_block_does_not_bleed() {
...
fn yuva422p_premultiplied_nonuniform_alpha_luma_is_native_y_not_colour() {
...
fn yuva422p_straight_and_premult_differ_under_varying_alpha() {
...
#[test]
fn yuva422p_default_alpha_mode_is_straight() {
  let sink = MixedSinker::<Yuva422p>::new(SRC, SRC);
...
fn yuva422p_identity_plan_matches_direct() {
  let (y, u, v, a) = planes(0x0F0F);
  let mut rgba = std::vec![0u8; SRC * SRC * 4];
  {
    let mut sink =
      MixedSinker::<Yuva422p, AreaResampler>::with_resampler(SRC, SRC, AreaResampler::to(SRC, SRC))
        .unwrap()
        .with_rgba(&mut rgba)
        .unwrap();
    yuva422p_to(&frame(&y, &u, &v, &a), FR, M, &mut sink).unwrap();
...
fn yuva422p_fractional_ratio_rgba_matches_oracle() {
  ...
  let mut u = std::vec![0u8; cw * S];
  let mut v = std::vec![0u8; cw * S];
  ...
  {
    let mut sink =
      MixedSinker::<Yuva422p, AreaResampler>::with_resampler(S, S, AreaResampler::to(O, O))
        .unwrap()
        .with_rgba(&mut rgba)
        .unwrap();
  ...
  {
    let mut sink = MixedSinker::<Yuva422p>::new(S, S)
      .with_rgba(&mut full)
      .unwrap();
    yuva422p_to(&src, FR, M, &mut sink).unwrap();
  }
...
fn yuva422p_limited_range_luma_is_native_y() {
...
fn yuva422p_cross_frame_reset_reuses_streams() {
...
fn yuva422p_accepts_alpha_mode_change_across_frames() {
  ...
  let mut rgba = std::vec![0u8; OUT * OUT * 4];
  {
    ...
    yuva422p_to(&frame(&y, &u, &v, &a), FR, M, &mut sink)
...
fn yuva422p_mid_frame_alpha_mode_flip_is_rejected() {
  ...
  // 4:2:2 — every luma row has its own chroma row.
  sink
    .process(Yuva422pRow::new(
      &y[..SRC],
      &u[..CW],
      &v[..CW],
      &a[..SRC],
...
#[test]
fn yuva422p_out_of_sequence_first_row_is_rejected() {
...
#[test]
fn yuva422p_no_output_sink_is_a_noop() {
...
fn yuva422p_resample_simd_matches_scalar() {
  ...
  assert_eq!(run(true), run(false), "Yuva422p resample SIMD != scalar");
}

#[test]
fn yuva422p_direct_hsv_only_is_rgb_free_and_infallible() {
  // #263 PR 8: on the direct (identity) path, `with_luma` +
  // `with_luma_u16` + `with_hsv` with NO rgb / rgba plane attached now
  // routes HSV through the direct `yuv_420_to_hsv_row` kernel (4:2:2
  // reuses the half-chroma 4:2:0 kernel) — it does NOT touch the growing
  // rgb scratch. Proof: arm the rgb-scratch allocation failpoint (which
  // would surface `AllocationFailed` if the path still grew the scratch);
  // the row must instead SUCCEED, leave the scratch unallocated, and write
  // every output. The failpoint is take-on-read, so disarm it after to
  // avoid leaking into a later same-thread test.
  ...
  // HSV row 0 matches the explicit YUV→RGB→HSV reference (4:2:0 kernel).
  let mut rgb0 = std::vec![0u8; SRC * 3];
  crate::row::yuv_420_to_rgb_row(&y[..SRC], &u[..CW], &v[..CW], &mut rgb0, SRC, M, FR, true);
