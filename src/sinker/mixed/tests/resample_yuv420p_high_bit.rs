//! Fused-downscale coverage for the high-bit **planar** 4:2:0 YUV family —
//! `Yuv420p10` / `Yuv420p12` / `Yuv420p14` / `Yuv420p16` (LE + BE wire).
//! Low-packed `u16` Y plane + half-width **and** half-height U / V planes
//! (horizontal chroma upsampling done in-register by the 4:2:0 row kernels;
...
//!   (BITS - 8)`. (Yuv420p exposes no `luma_u16` output.)
//!
//! Colour oracles drive a direct identity sink at source resolution (the
//! walker upsamples chroma both axes to full-res before the sink converts)
...
macro_rules! yuv420p_high_bit_resample_suite {
  (
    ...
    mod $mod {
      ...
      /// `SRC`-grid Y plane and half-width / half-height U / V planes
      /// (4:2:0, low-packed native codes so every kernel sees real math).
      ...
      fn chroma_upsample_matches_direct() {
        ...
        // horizontal upsample happens in-register inside the u8 / u16
        // conversion closures. A per-pixel-varying chroma frame must still
        // match the direct convert-then-bin (which upsamples the same way
        // both axes at full resolution).
        let (y, u, v) = ramp();
        let mut rgb = vec![0u8; OUT * OUT * 3];
        let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
        {
...
yuv420p_high_bit_resample_suite!(
  yuv420p10,
  Yuv420p10LeFrame,
  Yuv420p10BeFrame,
  Yuv420p10,
