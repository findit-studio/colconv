...
// dedicated `vuyx_to_rgba_row` kernel (α forced opaque) and never grows the
// scratch. Reuses the crate's `yuva`-gated RGB-scratch failpoint, so this test
// is too (under `--all-features` both `yuv-444-packed` and `yuva` are on).
...
fn vuyx_rgb_scratch_alloc_failure_leaves_outputs_untouched() {
  use crate::resample::ResampleError;

  let buf = solid_vuyx_frame(16, 8, 128, 128, 128, 0);
  let src = VuyxFrame::try_new(&buf, 16, 8, 16 * 4).unwrap();
  let n = 16 * 8;
  ...
  let mut sink = MixedSinker::<Vuyx>::new(16, 8)
    .with_luma(&mut luma)
    .unwrap()
    .with_rgba(&mut rgba)
    .unwrap()
  ...
  let err = vuyx_to(&src, false, ColorMatrix::Bt601, &mut sink).unwrap_err();
  drop(sink);
