//! Fused-downscale coverage for the high-bit **semi-planar** 4:2:2 YUV
//! family — `P210` / `P212` / `P216` (LE + BE wire). A full-width
//! high-bit-packed `u16` Y plane plus one **interleaved** half-width /
//! **full-height** `U,V,U,V…` plane; the row kernels de-interleave the UV
//! plane and nearest-neighbour upsample chroma horizontally in-register.
//! Unlike 4:2:0 there is no vertical chroma sharing — each luma row reads
//! its own (same-index) chroma row.
//!
//! These route through [`packed_yuv422_triple_resample`] with **three**
//! independent native-precision binnings (see `resample_p0xx_high_bit`
//! for the full contract — the only difference is full-height chroma):
...
//!   source-width native u16 RGB row (low-bit-packed) — never a narrowing.
//! - **luma** bins the **de-packed** native Y; `luma = binned_Y >>
//!   (BITS - 8)`. (P-formats expose no `luma_u16` output.)
//!
//! Colour oracles drive a direct identity P-format sink at source
//! resolution and 2x2-block-mean its output; the luma oracle area-bins
...
macro_rules! p2xx_high_bit_resample_suite {
  (
    ...
    mod $mod {
      ...
      /// `(U, V)` ramp, high-bit-packed into a full-width Y plane and a
      ...
      fn chroma_upsample_matches_direct() {
        // 4:2:2 chroma upsampling is horizontal-only (full-height chroma):
        // the UV de-interleave + horizontal nearest-neighbour upsample
        // happen in-register inside the conversion closures. A
        // per-pixel-varying chroma frame must still match the direct
        // convert-then-bin (which upsamples the same way at full
        // resolution).
        let (y, uv) = ramp();
        let mut rgb = vec![0u8; OUT * OUT * 3];
        let mut rgb_u16 = vec![0u16; OUT * OUT * 3];
      ...
      fn out_of_sequence_first_row_rejected_before_allocation() {
        ...
        // 4:2:2 chroma row == luma row; the interleaved chroma row is
        // `SRC` u16 wide (`2 * (SRC / 2)`).
      ...
      fn rejects_mid_frame_output_change() {
        ...
        .unwrap();
        sink.begin_frame(SRC as u32, SRC as u32).unwrap();
        // 4:2:2: each luma row reads its own (same-index) chroma row.
        sink
