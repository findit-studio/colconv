...
//!   α=`0xFF`) followed by
//!   `dispatch::alpha_extract::copy_alpha_packed_u8x4_at_3` to overwrite
...
impl<'a> MixedSinker<'a, Vuya> {
  ...
  /// `dispatch::alpha_extract::copy_alpha_packed_u8x4_at_3` overwrites
  ...
  fn process(&mut self, row: VuyaRow<'_>) -> Result<(), Self::Error> {
    ...
    if need_rgb_kernel {
      ...
      if want_rgba {
        ...
        crate::row::dispatch::alpha_extract::copy_alpha_packed_u8x4_at_3(packed, rgba_row, w);
      }
    }
