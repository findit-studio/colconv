//! Sinker impl for the packed VYU444 source format — 8-bit packed YUV
//! 4:4:4 with **no alpha** (24bpp).
//!
//! VYU444 (FFmpeg `AV_PIX_FMT_VYU444`) packs **three u8 bytes per pixel**
//! (`[V, Y, U]`). There is no fourth byte — no alpha and no padding; it is
//! the no-alpha sibling of [`Vuyx`](crate::source::Vuyx) with the padding
//! byte dropped entirely. RGBA outputs always force α to `0xFF`. The
//! packed slice type is `&[u8]`, with `3 x width` byte elements per row.
//! There is no chroma subsampling — every pixel carries its own
//! independent V / U / Y triplet (4:4:4).
//!
//! This sink mirrors the [`Vuyx`](crate::source::Vuyx) sink exactly; the
//! only differences are the 3-byte source stride and the V/Y/U byte
//! offsets (V at 0, Y at 1, U at 2).
//!
...
//! - `with_luma` — extracts the Y byte at offset 1 of each pixel
...
// guard, and `packed_vyu444_process_native` exists only when the reused 8-bit
...
use super::{NativeRouteChanged, packed_vyu444_process_native};
...
use crate::{
  PixelSink,
  row::{
    expand_rgb_to_rgba_row, rgb_to_hsv_row, vyu444_to_hsv_row, vyu444_to_luma_row,
    vyu444_to_luma_u16_row, vyu444_to_rgb_row, vyu444_to_rgba_row,
  },
  source::{Vyu444, Vyu444Row, Vyu444Sink},
};

impl<'a, R> MixedSinker<'a, Vyu444, R> {
  /// Attaches a **`u16`** luma output buffer. Y bytes from the packed VYU444
  /// `[V, Y, U]` layout are zero-extended to u16
  ...
  /// Attaches a packed **8-bit** RGBA output buffer. When VYU444 is the
...
impl<R> Vyu444Sink for MixedSinker<'_, Vyu444, R> {}

impl<R> PixelSink for MixedSinker<'_, Vyu444, R> {
  ...
  fn begin_frame(&mut self, width: u32, height: u32) -> Result<(), Self::Error> {
    ...
    // `Vyu444` exposes no u16 colour outputs, so its u16 colour streams are
  ...
  fn process(&mut self, row: Vyu444Row<'_>) -> Result<(), Self::Error> {
    const BITS: u32 = 8;
    let w = self.width;
    let h = self.height;
    let idx = row.row();
    ...
    // VYU444 row = `width x 3` bytes (one V/Y/U triple per pixel).
    let packed_expected =
      w.checked_mul(3)
    ...
    // Non-identity plan: `Vyu444` has NO alpha (24bpp, α forced opaque).
    // The span kind picks the engine:
    //
    // - `Area` bins through the no-alpha three-stream tail exactly like the
    //   no-alpha packed 4:4:4 YUV siblings (`Vuyx` / `V30X` / `V410` /
    //   `Xv36`) — u8 colour bins the converted u8 RGB row, native Y bins the
    //   de-interleaved Y plane (the colour outputs force α opaque).
    // - `Filter` runs the signed-coefficient filter on the converted RGBA
    //   via the shared packed-YUVA filter tail. `vyu444_to_rgba_row` writes a
    ...
    //   padding-byte filter path is needed. Straight alpha (`Vyu444` has no
    //   alpha mode).
    //
    // `Vyu444` exposes no u16 colour outputs, so the u16 colour resampling is
    ...
    if let Some(plan) = self.plan.as_ref() {
      ...
      if let crate::resample::SpanKind::Filter = plan.kind() {
        return packed_yuva444_filter_resample::<BITS, false, false>(
          rgba_filter_stream,
          rgba_filter_stream_u16,
          // Packed `Vyu444` never uses the u8 native-Y luma stream
          ...
          idx,
          use_simd,
          // Packed `Vyu444` routes luma through `deinterleave_y` + the u16
          // stream (no contiguous native-Y plane), so the u8-luma input and
          // its de-interleave scratch are unused.
          &[],
          None,
          // α forced opaque (`0xFF`) by `vyu444_to_rgba_row` — the padding
          // byte is never read as alpha.
          |dst| vyu444_to_rgba_row(packed, dst, w, matrix, full_range, use_simd),
          // `Vyu444` has no u16 colour outputs, so this closure is never called.
          |_dst: &mut [u16]| {},
          |dst| vyu444_to_luma_u16_row(packed, dst, w, use_simd),
          // u16-luma path, so this u8 de-interleave is never called.
          |_dst: &mut [u8]| {},
        );
      }
      // Area plan. When the native tier is enabled (and the planar join it
      // reuses is compiled in), de-pack the interleaved 3-byte `V Y U` row
      // into Y / U / V scratch (4:4:4, full-width chroma) and bin those
      // planes at output resolution, converting once per output row (Vyu444:
      // V at 0 / Y at 1 / U at 2). Otherwise (or under `with_native(false)`)
      ...
      packed_yuv444_triple_resample::<BITS>(
        ...
        matrix,
        full_range,
        |scratch| vyu444_to_rgb_row(packed, scratch, w, matrix, full_range, use_simd),
        // `Vyu444` has no u16 colour outputs, so `need_u16_color` is always
        // false in the tail and this closure is never called.
        |_scratch: &mut [u16]| {},
        |scratch| vyu444_to_luma_u16_row(packed, scratch, w, use_simd),
      )?;
      #[cfg(all(feature = "yuv-444-packed", feature = "yuv-planar"))]
      {
        let need_output =
    ...
    // `vyu444_to_rgba_row` kernel — α forced opaque). The later
    // `need_rgb_kernel` decode reuses the already-sized buffer.
    ...
    // Luma u8 — extract Y byte (offset 1 in each 3-byte V/Y/U triple).
    ...
    // ===== u8 RGB / RGBA / HSV path (Strategy A for VYU444) =====
    // HSV-without-RGB-or-RGBA goes through the direct `vyu444_to_hsv_row`
    ...
    // α is forced to `0xFF` by `vyu444_to_rgba_row` (ALPHA_SRC = false).
    ...
    vyu444_to_rgb_row(packed, rgb_row, w, row.matrix(), row.full_range(), use_simd);
    ...
    // row instead of running a second YUV→RGB kernel. For VYU444,
    // α=`0xFF` is semantically correct in both paths (the source carries
    // no alpha), so Strategy A applies (spec § 8.4).
